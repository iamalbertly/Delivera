...
- **Actions:** One owner cluster per person — primary **Draft nudge** (guided text via `buildGuidedNudgeText`); issues collapsed by default. Command **Do first** scrolls to the first cluster when clusters exist (no duplicate external Open link). Setup gaps cap at **one** card above the fold with expand-in-place.
