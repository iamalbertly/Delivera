...
- **Report chrome SSOT:** `tests/Delivera-Report-Chrome-Direct-Value-Realtime-Validation-Tests.spec.js` (journey `journey.ux-core` via `Delivera-Tests-Journey-Buckets-Map-SSOT.js`) stubs `/preview.json` and asserts a single header context bar, compact preview meta, signals-rail layout, compact sidebar trust card, and that `body.preview-active` tracks the visible preview shell (`syncReportPreviewActiveFromDom` + test-only hook when `__DELIVERA_TEST_DISABLE_AUTO_PREVIEW` is set). Shared `ensureReportFiltersVisible` in `Delivera-Tests-Shared-PreviewExport-Helpers.js` opens collapsed report filters deterministically for other specs too.
