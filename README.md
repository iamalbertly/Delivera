...
- **Layout contract:** `body.has-top-chrome` owns `--sticky-global-nav-top` (nav stack only — use for the first sticky layer: scope bar / sprint HUD) and `--sticky-offset` (nav + scope bar — use for drawers/rails **below** the scope bar). Never apply `--sticky-offset` as `top` on the scope bar itself (double-count gap). Live heights via `Delivera-Shared-Sticky-Offset-01Measure.js`.
