...
**Portfolio command surface (`/governance`):** Priority Brief hero (`#governance-priority-surface-mount`) is the **single visible paint path** — composed by `Delivera-App-Governance-PrioritySurface-01Render-UI.js` (hero + agentic + exception + commitment detail + carousel slot). Hidden legacy mounts (`#portfolio-signal-mount`, commitments, prepared-actions, carousel mount, decision panel) are **not repainted** on `governance-priority-brief-page` so they cannot layer over live UI. **Status grammar SSOT:** `Delivera-App-Portfolio-CardStatus-01Gradation-SSOT.js` (shared by bento cards + displacement copy). **Actions scope filter SSOT:** `Delivera-App-Actions-Case-01Scope-Filter-SSOT.js`. **Instant loading:** static first-paint shells in HTML + `Delivera-Shared-Instant-Shell-01UI.js` (sessionStorage stale-while-revalidate) + compact `.gov-scope-stale-overlay` chip — never a blank white main. Orphan parallel modules marked `DeleteThisFile_*` after import validation. Pipeline: `Page-03Load-Controller.js` + `Page-06Portfolio-Render-Plugin.js` + Priority Brief modules + `PIBaseline` wizard + `routes/Delivera-Governance-PIBaseline-01Routes.js`. **Canonical P0 gate:** `npm run test:journey:core` (alias for `test:journey:churn-trust-repair` — 31 units + 37 Playwright, fail-fast `--max-failures=1`). Legacy: `test:journey:governance-priority-brief` · `test:journey:portfolio` (subset of core).
