...
- **PI baseline:** Epic-first propose/compare; **Create work** on empty baseline, setup gaps, and ad-hoc epic drawer (`data-open-outcome-modal`). Wizard: `GET /api/governance/pi-baseline/propose?quarter=` → confirm (`POST /api/governance/pi-baseline`). **Slide upload:** drag-and-drop or click on Brief wizard and Create work (`POST /api/governance/pi-baseline/propose-from-image`) — needs **OpenAI or Claude** key in **Settings** (browser `localStorage`) or server `.env`; Gemini is rejected for vision. Unmatched slide bullets stay visible for manual Create work. Epic **activity** labels (not started / in flight / sprint count) come from cached board data via `Delivera-Governance-PIBaseline-04Epic-Activity-Intelligence-SSOT.js`. Quarter persists in `delivera_gov_quarter_v1`.
- **Actions:** DOM order is answer → **owner clusters** → compact setup link → folded portfolio/squad verdict → PI strip (`.gov-pi-strip-fold` closed when clusters exist). One cluster per person — primary **Draft nudge** (guided text via `buildGuidedNudgeText`); issues collapsed by default. Command **Do first** scrolls to the first cluster when clusters exist (no duplicate external Open link). When clusters exist, setup gaps collapse to **N setup gaps — show**; portfolio heat / squad verdict fold closed by default (status stays in command blocks). **Supporting evidence** (`#gov-supporting-evidence`) holds measurement strip + meeting script (below the fold by default).
...
- **Sprint projects:** `current-sprint.html` hydrates `#current-sprint-projects` via `Delivera-CurrentSprint-Projects-Catalog-01Hydrate.js` (same catalog SSOT; no hardcoded `<option>` rows).
- **Notifications:** Bell mounts `#app-notification-dock` inside `#app-notification-slot` (`12-top-chrome.css`). Brief context link: **Open Brief queue**. Sprint/Report/Leadership init call `refreshNotificationDockFromStore()` (no duplicate `renderNotificationDock` on chrome pages). Legacy dock CSS scoped to `body:not(.has-top-chrome)` in `04-filters-report.css` and `07-leadership.css`.
...
- **Above-fold clutter:** Feedback lab and micro-survey live in collapsed **Feedback & survey** details; scope capsule collapsed until **Change**; proof **Draft nudge** is link-style; trusted PI strips hide counter rows and keep timeline in closed details. On mobile with owner clusters, agent queue chrome, freshness, trust chips, and duplicate answer blocks hide so the first **Draft nudge** stays above the fold (`375×812` Playwright check). Brief does **not** fetch `/api/governance/pi-confidence` on load (uses `brief.meta` only); scope drawer uses `brief.meta.scopeIntelligence`.
- **Deferred:** per-user access index sharding; monthly Jira `project/search` diff vs catalog; baseline confirm → Jira write-back (fixVersion/label API); CI `test:all` with warm server after API changes. Smart-dev maps governance paths in `scripts/Delivera-Test-Selection-Helper.js`.
