...
- **Above-fold clutter:** Feedback lab and micro-survey live in collapsed **Feedback & survey** details; scope capsule collapsed until **Change**; proof **Draft nudge** is link-style; trusted PI strips hide counter rows and keep timeline in closed details; mobile hides the third answer block. Brief does **not** fetch `/api/governance/pi-confidence` on load (uses `brief.meta` only); scope drawer uses `brief.meta.scopeIntelligence`.
- **Deferred:** per-user access index sharding; monthly Jira `project/search` diff vs catalog; current-sprint catalog hydrate (Report SSOT); baseline confirm → Jira write-back (fixVersion/label API); CI `test:all` with warm server after API changes.
