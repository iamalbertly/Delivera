...
/**
 * Derive the current Vodacom fiscal quarter label from a date.
 * Reuses the same quarter-numbering as getQuartersUpToCurrent.
 * (Audit 2026-07-15: period.vodacomQuarter was always null.)
 * @param {Date} date
 * @returns {string} e.g. "FY27 Q2"
 */
export function deriveVodacomQuarterFromDate(date = new Date()) {
  const y = date.getUTCFullYear();
  const m = date.getUTCMonth();
  let qNum, qYear;
  if (m <= 2) { qNum = 4; qYear = y; }
  else if (m <= 5) { qNum = 1; qYear = y; }
  else if (m <= 8) { qNum = 2; qYear = y; }
  else { qNum = 3; qYear = y; }
  const range = getQuarterRange(qNum, qYear);
  if (!range) return '';
  const end = new Date(range.endISO);
  const fyInfo = getFiscalYearLabel(end);
  return fyInfo.shortLabel ? `${fyInfo.shortLabel} Q${qNum}` : `Q${qNum} ${qYear}`;
}
