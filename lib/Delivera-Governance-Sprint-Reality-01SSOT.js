...
function businessDaysRemaining(from, to) {
...
export function projectSquadSprintTruth(input = {}, { now = new Date() } = {}) {
  ...
  const remainingBusinessDays = endDate ? businessDaysRemaining(now, endDate) : null;
  ...
  const facts = [];
  if (state === 'active') facts.push(`${name || 'Sprint'} is active${remainingBusinessDays != null ? `, ${remainingBusinessDays} business days remaining` : ageDays ? `, started ${ageDays} days ago` : ''}.`);
  ...
  projected.payloadHash = createHash('sha256').update(JSON.stringify({
    state: projected.state,
    sprintId: projected.sprintId,
    sprintName: projected.sprintName,
    startDate: projected.startDate,
    endDate: projected.endDate,
    unmoved: projected.unmoved,
    carryoverCount: projected.carryoverCount,
    reopened: projected.reopened,
    checkedBoards: projected.checkedBoards,
    evidenceAt: projected.evidenceAt,
  })).digest('hex');
  projected.version = clean(input?.sprintReality?.version || meta.sprintTruthVersion || projected.evidenceAt, 100);
  return projected;
}
