...
export function buildSprintPulseFromPayload(payload) {
  ...
  const daysElapsedCalendar = days.daysElapsedCalendar ?? null;
  ...
  return { done, committed, pct, daysElapsed, daysElapsedCalendar, daysRemaining, phaseHint };
}
...
export function buildSquadInsight(projectKey, entries = [], contract, evidencePack = {}, opts = {}) {
  ...
  // Prefer catalog label for boardName — a shared/foreign Jira board must not
  // rename the squad. (Audit 2026-07-15: "COPS PROJECT" phantom squad bug.)
  const catalogLabel = catalogEntry(PK)?.label || '';
  const rawBoardName = payload?.board?.name || activeEntry?.board?.name || '';
  const boardName = catalogLabel || rawBoardName;
