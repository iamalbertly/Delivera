 ...
 */
export function buildSquadInsight(projectKey, entries = [], contract, evidencePack = {}) {
  const PK = String(projectKey || '').trim().toUpperCase();
  ...
  return {
    ...
    productivityLine: stale > 0 ? 'Stale work detected — squad may be stuck' : 'Productivity looks healthy',
...
export function buildPortfolioRollup(squadInsights = []) {
  ...
  return { totalSquads, blockerCount, bottleneckCount, summaryLine };
}

export function buildSquadInsights(contract, boardPayloads, evidencePack = {}) {
  ...
  return projects.map((p) => buildSquadInsight(p, grouped.get(String(p).trim().toUpperCase()) || [], contract, evidencePack));
}

export function attachPortfolioInsightsToBrief(contract, boardPayloads, evidencePack = {}) {
  const squadInsights = buildSquadInsights(contract, boardPayloads, evidencePack);
...
export function attachExecutiveViewToBrief(contract, boardPayloads, evidencePack) {
  ...
  attachPortfolioInsightsToBrief(contract, boardPayloads, evidencePack);
