...
export function buildSquadInsight(projectKey, entries = [], contract, evidencePack = {}, opts = {}) {
  ...
  if (!boardResolved || !payload) {
    return {
      ...
      sprintReality: projectSquadSprintTruth({ checkedBoards: entries.map((entry) => ({ id: entry?.board?.id, verified: Boolean(entry?.payload) })) }),
  ...
  const sprintReality = projectSquadSprintTruth({ ...payload, checkedBoards: entries.map((entry) => ({ id: entry?.board?.id, name: entry?.board?.name, verified: Boolean(entry?.payload) })) });
