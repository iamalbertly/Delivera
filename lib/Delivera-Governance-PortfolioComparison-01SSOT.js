...
export function buildPortfolioComparisonCards({
  ...
  const rankReasonByProject = new Map((decision.comparisonReason || []).map((row) => [
    String(row.projectKey || '').toUpperCase(),
    row.reason || '',
  ]));
  ...
  for (const key of order) {
    ...
    cards.push({
      ...
      rankReason: rankReasonByProject.get(key)
        || (gated ? readiness.reason : squadExplanation(insight, delivery, offPlan, proof, squadCases.length)),
  ...
  const remainingCards = cards.slice(maxVisible);
  return {
    cards: visibleCards,
    remainingCards,
    allCardCount: cards.length,
    overflowCount: remainingCards.length,
    lazyLoad: {
      enabled: remainingCards.length > 0,
      pageSize: 3,
      loaded: visibleCards.length,
      remaining: remainingCards.length,
      trigger: 'horizontal-scroll-end',
    },
    compareModes: [
      { id: 'backlog-readiness', label: 'Backlog readiness', meaning: 'Which squads have PI commitments backed by Jira stories?' },
      { id: 'pi-evidence', label: 'PI evidence', meaning: 'Which squads have uploaded current-quarter commitments?' },
      { id: 'owner-risk', label: 'Owner risk', meaning: 'Which decisions need a named owner now?' },
      { id: 'stakeholder-proof', label: 'Stakeholder proof', meaning: 'Which claims have acceptance or outcome evidence?' },
      { id: 'recovery-window', label: 'Recovery window', meaning: 'Which squads still have enough quarter time to recover?' },
    ],
