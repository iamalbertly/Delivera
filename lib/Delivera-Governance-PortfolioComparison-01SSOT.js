...
function squadMainIssue(insight = {}, delivery = 0, offPlan = 0, proof = 0) {
  ...
  if (offPlan >= 35) return 'Off-plan load';
...
function squadExplanation(insight = {}, delivery = 0, offPlan = 0, proof = 0, openCases = 0) {
  ...
  if (offPlan >= 40 && delivery < 45) {
    return `${name}: High off-plan work (${offPlan}%) and weak proof (${proof}%) are driving low delivery.`;
  }
  ...
  if (offPlan >= 35) {
    return `${name}: Off-plan work is high (${offPlan}%) and committed delivery remains ${delivery}%.`;
  }
  if (proof < 50) {
...
export function buildPortfolioComparisonCards({
  ...
  for (const key of order) {
    ...
    cards.push({
      ...
      mainIssue: squadMainIssue(insight, delivery, offPlan, proof),
      affectedCommitmentCount: squadCommitments.length || commitments,
      decisionNeeded: cardDecisionNeeded(insight, delivery, proof),
      nextAction: cardNextAction(insight, nudgesForSquad),
      explanation: squadExplanation(insight, delivery, offPlan, proof, squadCases.length),
      proofDetail: `Delivered ${delivery}% · Off-plan ${offPlan}% · Proof ${proof}% · ${commitments} commitment${commitments === 1 ? '' : 's'}`,
