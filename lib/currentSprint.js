...
import { getCachedTanzaniaHolidays, preloadTanzaniaHolidays } from './Delivera-Data-TanzaniaHolidays-01SSOT.js';
...
import { listPIBaselines } from './Delivera-Governance-PIBaseline-01Store-IO.js';
import {
  buildFlowBaseline,
  enhanceFlowIntervention,
} from './Delivera-CurrentSprint-Flow-Intelligence-SSOT.js';
...
function computeDaysMeta(sprint, now = new Date()) {
  ...
  // Best-effort holiday awareness: use cached TZ holidays (preloaded at startup)
  const year = start.getFullYear();
  const holidays = getCachedTanzaniaHolidays(year);

  const calendarDays = Math.ceil((end - start) / (24 * 60 * 60 * 1000));
  const workingDays = calculateWorkDays(sprint.startDate, sprint.endDate, holidays);
  ...
  if (nowTime >= startTime && nowTime <= endTime) {
    ...
    daysElapsedWorking = typeof workingDaysNum === 'number' ? calculateWorkDays(elapsedStart, elapsedEnd, holidays) : null;
    daysRemainingWorking = typeof workingDaysNum === 'number' ? calculateWorkDays(remainingStart, remainingEnd, holidays) : null;
...
function computeStoriesList(issues, storyPointsFieldId, epicLinkFieldId) {
  ...
  for (const issue of issues) {
    ...
    children.push({
      ...
      hoursInStatus: computeHoursSinceIso(
        issue.fields?.statuscategorychangedate || issue.fields?.updated || issue.fields?.created || ''
      ),
  ...
  for (const issue of issues) {
    ...
    stories.push({
      ...
      statusChangedAt: issue.fields?.statuscategorychangedate || issue.fields?.updated || '',
      issueType: issue.fields?.issuetype?.name || '',
      reporter: issue.fields?.reporter?.displayName || '',
      assignee: issue.fields?.assignee?.displayName || '',
      created: issue.fields?.created || '',
      ageHours: computeHoursSinceIso(issue.fields?.created || ''),
      ...
      subtasks,
      issueLinks: Array.isArray(issue.fields?.issuelinks) ? issue.fields.issuelinks : [],
      issueUrl: buildIssueUrl(issue.key || ''),
    });
  }
...
function buildDecisionCockpit({
  ...
  function ensureRiskEntry(issueKey, seed = {}) {
    ...
    if (!riskRegistry.has(key)) {
      riskRegistry.set(key, {
        ...
        hoursInStatus: safeNumber(seed.hoursInStatus, 0),
        storyPoints: safeNumber(seed.storyPoints, 0),
        tags: [],
        riskTags: [],
        score: 0,
        reason: '',
  ...
  (Array.isArray(stuckCandidates) ? stuckCandidates : []).forEach((issue) => {
    ...
    const isActiveBlocker = status.includes('in progress') || status.includes('blocked');
    addRisk(entry, {
      tag: 'No movement 24h',
      riskTag: isActiveBlocker ? 'blocker' : 'stale',
      score: isActiveBlocker ? 92 : 50,
      reason: isActiveBlocker
        ? `The item has been stale for ${roundOne(issue.hoursInStatus || 24)}h and is likely blocking sprint flow.`
        : `The item has not moved for ${roundOne(issue.hoursInStatus || 24)}h and needs an owner-ready next step.`,
    });
  });
  ...
  const topRisks = [...riskRegistry.values()]
    ...
    .map((entry) => {
      ...
      return {
        ...
        hoursInStatus: entry.hoursInStatus,
  ...
  const activeBlockerCount = topRisks.filter((risk) => risk.riskTags.includes('blocker')).length;
  const nextBestAction = nextBestActionRisk
    ? {
        ...
        assignee: nextBestActionRisk.assignee,
        hoursInStatus: nextBestActionRisk.hoursInStatus,
        businessImpact: nextBestActionRisk.riskTags.includes('blocker')
          ? 'The sprint outcome is at risk while this work blocks flow.'
          : nextBestActionRisk.riskTags.includes('unassigned')
            ? 'Unowned sprint work can miss the commitment without an accountable next move.'
            : nextBestActionRisk.riskTags.includes('scope')
              ? 'Late scope consumes capacity that was committed to planned outcomes.'
              : 'Weak execution evidence reduces delivery predictability.',
  ...
  if (activeBlockerCount > 0) {
    ...
    healthTone = 'critical';
  } else if (topRisks.length > 0 || scopeAddedSp > 0 || inactivity || percentDone < 40) {
...
export async function buildCurrentSprintPayload({ board, projectKeys, agileClient, fields, options = {} }) {
  ...
  let sprints = [];
  let sprintFetchSkipped = false;
  let sprintFetchReason = '';
  try {
    sprints = await fetchSprintsForBoard(boardId, agileClient, { board });
  } catch (err) {
    sprintFetchSkipped = true;
    sprintFetchReason = 'jira-error';
    sprints = [];
  }
  if (!sprintFetchSkipped && Array.isArray(sprints) && sprints.length === 0 && board?.type && String(board.type).toLowerCase() !== 'scrum') {
    sprintFetchSkipped = true;
    sprintFetchReason = 'non-scrum';
  }
  ...
  if (!sprint) {
    return {
      ...
      flags: sprintFetchSkipped ? { sprintFetchSkipped: true, reason: sprintFetchReason || 'no-active-sprint' } : null,
      daysMeta: null,
      ...
      meta: {
        ...
        jiraHostResolved,
        ...(sprintFetchSkipped ? { sprintFetchSkipped: true, sprintFetchReason } : {}),
      },
    };
  }
  ...
  try {
    ...
    const eligible = closed
      .filter((candidate) => candidate.id !== sprint.id
        && (!currentEnd || new Date(candidate.endDate || 0).getTime() < currentEnd))
      .slice(0, 6);
    historicalSprintSamples = await Promise.all(eligible.map(async (candidate) => ({
      sprint: candidate,
      issues: await fetchSprintIssuesForTransparency(
        candidate.id,
        agileClient,
        projectKeys || [board.location?.projectKey].filter(Boolean),
        getDefaultPeerWorkItemTypes(),
    ...
    const priorSample = historicalSprintSamples[0];
    if (priorSample) {
      const prior = priorSample.sprint;
      const prevIssues = priorSample.issues;
    ...
    logger.warn('Historical sprint flow baseline skipped', { boardId, error: err?.message });
  }

  const flowBaseline = buildFlowBaseline(historicalSprintSamples);
  let approvedCommitments = [];
  try {
    const projectSet = new Set((projectKeys || []).map((key) => String(key || '').trim().toUpperCase()));
    const baselines = await listPIBaselines();
    const fiscalYear = String(sprint?.name || '').toUpperCase().match(/\bFY\d{2}\b/)?.[0] || '';
    const projectBaselines = baselines.filter((row) => (row?.projects || [])
      .some((key) => projectSet.has(String(key || '').toUpperCase())));
    const baseline = projectBaselines.find((row) => fiscalYear
      && String(row?.piName || row?.fiscalPeriod || '').toUpperCase().includes(fiscalYear))
      || projectBaselines[0];
    approvedCommitments = Array.isArray(baseline?.committedItems) ? baseline.committedItems : [];
  } catch (err) {
    logger.warn('Approved PI value mapping unavailable for sprint flow', { boardId, error: err?.message });
  }
  ...
  const payload = {
    ...
    stories,
    decisionCockpit: enhanceFlowIntervention({
      cockpit: buildDecisionCockpit({
        sprint,
        summary,
        stories,
        stuckCandidates,
      ...
      daysRemaining: daysMeta?.daysRemainingWorking ?? daysMeta?.daysRemainingCalendar ?? null,
      observedAt: new Date().toISOString(),
      context: { squadKey: projectKeys?.[0] || '' },
      sprint,
      partialPermissions: false,
      stale: false,
    }),
    ...
    meta: {
      ...
      jiraHostResolved,
      teamRoster,
      piBaselineCommittedKeys: approvedCommitments
        .map((item) => String(item?.issueKey || '').trim().toUpperCase())
        .filter(Boolean),
    },
  };
