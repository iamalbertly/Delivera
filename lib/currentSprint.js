...
function buildIssueUrl(issueKey) {
  const host = resolveJiraHostFromEnv();
...
export async function buildCurrentSprintPayload({ board, projectKeys, agileClient, fields, options = {} }) {
  ...
  const parentsWithActiveSubtasks = new Set(
    (subtaskTracking.subtasks || [])
      .filter((row) => row && row.statusCategoryKey !== 'done' && row.hoursInStatus != null && row.hoursInStatus < 24)
  ...
  const stuckCandidates = issues
    ...
    .filter((issue) => {
      ...
      if (issueKey && parentKeysInSprint.has(issueKey) && parentsWithActiveSubtasks.has(issueKey)) {
