...
function mergeOverrides(layers) {
  ...
  for (const row of all) {
    ...
    if (k === 'boardAlias' && row.aliasKey) boardAliases[String(row.aliasKey).toUpperCase()] = String(v);
    if (k === 'operatingModel' && row.aliasKey) operatingModels[String(row.aliasKey).toUpperCase()] = String(v);
...
export async function resolveEffectiveGovernanceProfile({
  ...
  return {
    ...
    boardAliases: merged.boardAliases,
    operatingModels: merged.operatingModels,
