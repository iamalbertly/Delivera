...
export function classifyFeedbackToAgent(entry = {}) {
  ...
  const note = String(entry.note || entry.phrase || entry.message || '').toLowerCase();
  const page = String(entry.page || entry.context?.page || '').toLowerCase();
  if (page === 'governance' && /brief|portfolio|squad|pi|verdict/.test(note)) return 'pi';
  if (page === 'governance' && /layout|scroll|click|ui|space/.test(note)) return 'ux';
...
export async function buildFeedbackTriageSummary({ project = '' } = {}) {
  ...
  const recentUserFeedback = userSubmissions.map((row, idx) => {
    ...
    const page = ctx.page || '';
    const agent = classifyFeedbackToAgent({ ...row, page, context: ctx });
    return {
