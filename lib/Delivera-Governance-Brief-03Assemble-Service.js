...
import { readRecentJobs, readLastCompletedBriefJob } from './Delivera-Governance-Worker-02Jobs-IO.js';
...
export async function assembleGovernanceBrief({
  ...
  if (baselineComparison) enrichDeliveryTruthFromBaseline(contract, baselineComparison);
  ...
  const workerReceipt = await buildWorkerReceipt(contract, {}, jobs);
