...
import { filterSprintCapableBoards, filterNonSprintCapableBoards } from './Delivera-Data-Board-Sprint-Capability-01SSOT.js';
...
// Portfolio coverage and Jira refresh capacity are separate concerns. Never hide
// registered squads merely to protect Jira: refresh every selected board through
// a small bounded worker pool and let the existing cache/single-flight layer fan
// the result out to readers.
const MAX_PORTFOLIO_BOARDS = 24;
const BOARD_REFRESH_CONCURRENCY = 3;
...
async function fetchBoardPayloads({ boards, projects, agileClient, fields }) {
  ...
  const skippedNonScrum = filterNonSprintCapableBoards(incoming);
  const active = filterSprintCapableBoards(incoming).slice(0, MAX_PORTFOLIO_BOARDS);
  const settled = [];
  for (let offset = 0; offset < active.length; offset += BOARD_REFRESH_CONCURRENCY) {
    const chunk = active.slice(offset, offset + BOARD_REFRESH_CONCURRENCY);
    const results = await Promise.allSettled(chunk.map((board) => buildCurrentSprintPayload({
  ...
  return {
    ...
    total: active.length,
    fromSnapshot,
    partialBoardSkips: skippedNonScrum.map((b) => ({
      id: b.id,
      name: b.name || '',
      type: b.type || 'unknown',
      reason: 'non-scrum',
...
export async function assembleGovernanceBrief({
  ...
  const { boardPayloads, failures, total, fromSnapshot, partialBoardSkips = [] } = await fetchBoardPayloads({ boards, projects, agileClient, fields });
  ...
  const freshnessMeta = {
    stale: false,
    partial: (failures > 0 && boardPayloads.length > 0) || partialBoardSkips.length > 0,
    fromSnapshot,
    fromCache: false,
  ...
  if (baselineComparison) {
    ...
    const activityForBaseline = buildEpicActivityByKey(boardPayloads);
    if (Array.isArray(baselineComparison.items)) {
      baselineComparison.items = baselineComparison.items.map((item) => {
        const act = activityForBaseline.get(String(item.issueKey || '').toUpperCase());
        if (!act) return item;
        return { ...item, epicActivity: item.epicActivity || act };
      });
    }
  ...
  const boardEpicIndex = collectEpicsFromPayloads(boardPayloads).map((e) => {
    const key = String(e.issueKey || '').trim().toUpperCase();
    const act = activityByKey.get(key) || null;
    return {
      issueKey: e.issueKey,
      title: e.summary,
      squad: e.squad,
      ...
      plannedStartDate: act?.firstActiveSprintStart || '',
  ...
  const piBaselineCommittedKeys = contract.meta?.piBaselineCommittedKeys
    || baseline?.committedItems?.map((i) => i.issueKey)
    || [];
