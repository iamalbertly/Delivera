...
export async function buildEvidencePack({
  ...
  for (let i = 0; i < bounded.length; i += DEFAULT_CONCURRENCY) {
    ...
    const settled = await Promise.allSettled(chunk.map(async (risk) => {
      try {
        const issue = await fetchIssueWithChangelog(version3Client, risk.issueKey, cache, cacheTtlMs);
        fetched += 1;
        return buildEvidenceRow(risk, issue, startFor(risk.issueKey));
      } catch (err) {
        logger.warn('Evidence changelog fetch failed', { issueKey: risk.issueKey, error: err?.message });
        degraded = true;
