...
export async function discoverBoardsForProjects(projectKeys, agileClient) {
  ...
  for (const projectKey of projectKeys) {
    ...
    try {
      ...
      const { status, code, userMessage } = classifyJiraHttpError(error);
      const logPayload = { projectKey, code, status, message: String(error?.message || '') };
      if (status === 401 || status === 403 || status === 404) {
        logger.warn(`Board discovery degraded for project ${projectKey}`, logPayload);
      } else {
