...
export function buildReportUrlFromContext(context = {}, fallbackPath = '/governance') {
  const normalized = normalizeReportContext(context);
  ...
  const fallback = fallbackPath || '/governance';
  const path = fallback.startsWith('/report')
    ? (normalized.reportPath || fallback)
    : fallback;
