...
function squadMeaning({
  ...
  if (baselineMissing || dataTrust === DATA_TRUST.BOARD_HEALTH_ONLY) {
    return 'Board health · plan not uploaded';
  }
  ...
  if (unsupportedCount > 0) {
    if (notPlannedCount > 0 && notPlannedCount >= unsupportedCount) {
      return `${notPlannedCount} committed · zero stories on selected boards`;
    }
    return `${linkedCount} linked · ${unsupportedCount} need action`;
  }
...
export function rankPortfolioSquads({
  ...
  for (const insight of insights) {
    ...
    if (!projectKey || projectKey === PORTFOLIO_ALL) continue;
