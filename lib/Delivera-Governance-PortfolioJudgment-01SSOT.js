...
function squadMeaning({ attentionState, unsupportedCount = 0, aligned = '', openCases = 0, baselineMissing = false, boardResolved = true } = {}) {
  if (baselineMissing) return 'Upload PI baseline slide for this quarter';
  if (attentionState === ATTENTION_STATES.CANNOT_VERIFY && boardResolved === false) return 'Board not mapped to Jira';
  if (attentionState === ATTENTION_STATES.CANNOT_VERIFY) return 'Commitments cannot be verified yet';
  ...
  if (unsupportedCount > 0) return `${unsupportedCount} promise${unsupportedCount === 1 ? '' : 's'} unsupported`;
