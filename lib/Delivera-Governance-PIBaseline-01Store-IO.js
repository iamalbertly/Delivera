...
function quarterMatches(piName = '', quarter = '') {
  const qNorm = String(quarter || '').replace(/\s+/g, ' ').trim().toUpperCase();
  if (!qNorm) return true;
  const pi = String(piName || '').toUpperCase();
  const qShort = qNorm.replace(/\s+/g, '');
  return pi.includes(qShort) || pi.includes(qNorm);
}

/**
 * Latest baseline per project for the active quarter.
 * A squad only inherits a slide whose `projects` list includes that project key.
 * @returns {Promise<Map<string, object|null>>}
 */
export async function getLatestPIBaselinesByProject({ projects = [], quarter = '' } = {}) {
  const pks = (Array.isArray(projects) ? projects : [])
    .map((p) => String(p || '').trim().toUpperCase())
    .filter(Boolean);
  const result = new Map();
  for (const pk of pks) {
    const list = await listPIBaselines({ project: pk });
    let best = null;
    for (const row of list) {
      if (!(row.committedItems || []).length) continue;
      if (!quarterMatches(row.piName, quarter)) continue;
      best = row;
      break;
    }
    if (!best) {
...
/**
 * Resolve latest baseline for a scope — join-key first, then project overlap + quarter hint.
 * Prefer getLatestPIBaselinesByProject for per-squad governance truth.
 */
export async function getLatestPIBaselineForScope({ projects = [], quarter = '' } = {}) {
  const pks = (Array.isArray(projects) ? projects : [])
    .map((p) => String(p || '').trim().toUpperCase())
    .filter(Boolean);
  const joinKey = pks.join('+');
  if (joinKey) {
    const exact = await getLatestPIBaseline(joinKey);
    if (exact?.committedItems?.length) return exact;
  }
  const byProject = await getLatestPIBaselinesByProject({ projects: pks, quarter });
  for (const pk of pks) {
    const row = byProject.get(pk);
    if (row?.committedItems?.length) return row;
  }
  const all = await listPIBaselines();
  for (const row of all) {
    const rowProjects = (row.projects || []).map((p) => String(p).toUpperCase());
    const projectHit = !pks.length || pks.some((pk) => rowProjects.includes(pk));
    if (!projectHit || !(row.committedItems || []).length) continue;
    if (!quarterMatches(row.piName, quarter)) continue;
    return row;
  }
  return null;
