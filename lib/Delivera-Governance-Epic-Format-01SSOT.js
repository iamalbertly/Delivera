/**
 * SSOT: Org epic naming format — FY## Q# – system – subsystem – [module] – capability.
 * Empty {module} is stripped so DMS stays 4 segments; FIN/TowerCo uses 5.
 */
export const EPIC_DELIM = ' – ';

/** Squad nicknames / aliases → project/system key (Tycoons → FIN). */
export const SQUAD_NICKNAME_ALIASES = Object.freeze({
  TYCOONS: 'FIN',
  'FINANCE SQUAD': 'FIN',
  FINANCE: 'FIN',
  DMS: 'DMS',
...
export const DEFAULT_EPIC_FORMAT = Object.freeze({
  template: '{quarter} – {system} – {subsystem} – {module} – {capability}',
...
export function normalizeEpicFormatConfig(raw = {}) {
  const base = {
    ...DEFAULT_EPIC_FORMAT,
    squadSystemMap: { ...DEFAULT_EPIC_FORMAT.squadSystemMap },
  };
...
/** Resolve nickname or raw squad string to catalog/system key. */
export function resolveSquadAlias(squad = '') {
  const raw = String(squad || '').trim();
  if (!raw) return '';
  const key = raw.toUpperCase().replace(/\s+SQUAD$/i, '').trim();
  if (SQUAD_NICKNAME_ALIASES[key]) return SQUAD_NICKNAME_ALIASES[key];
  if (SQUAD_NICKNAME_ALIASES[raw.toUpperCase()]) return SQUAD_NICKNAME_ALIASES[raw.toUpperCase()];
  return key;
}

export function resolveSquadSystem(squad = '', format = DEFAULT_EPIC_FORMAT) {
  const key = resolveSquadAlias(squad);
 ...
 * Empty module (and other empty tokens) are stripped after template substitution.
 */
...
/** 5-segment preview for FIN / ambition-table slides. */
export function buildEpicFormatPreviewWithModule(format = DEFAULT_EPIC_FORMAT) {
  return buildEpicTitleFromFormat({
    quarter: 'FY27 Q2',
    squad: 'FIN',
    subsystem: 'TOWERCO',
...
/**
 * Valid when FY## Q# then 3–4 additional non-empty segments (4 or 5 parts total).
 */
export function epicTitleRegexFromFormat(format = DEFAULT_EPIC_FORMAT) {
  const delim = String(format?.delimiter || EPIC_DELIM).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  // quarter + 3 or 4 segments (allows optional module)
  return new RegExp(
    `^FY\\d{2}\\s+Q\\d\\s*${delim}\\s*\\S.+?(?:${delim}.+){2,3}$`,
    'i',
  );
}
...
export function validateEpicTitle(title = '', format = DEFAULT_EPIC_FORMAT) {
  ...
  const delim = format.delimiter || EPIC_DELIM;
  const parts = t.split(new RegExp(`\\s*${delim.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*|\\s*[-–]\\s*`))
    .map((p) => p.trim())
    .filter(Boolean);
  if (/^FY\d{2}\s+Q\d/i.test(parts[0] || '') && (parts.length === 4 || parts.length === 5)) {
    return { valid: true };
  }
...
export function slideVisionNamingRule(format = DEFAULT_EPIC_FORMAT) {
  const preview4 = buildEpicFormatPreview(format);
  const preview5 = buildEpicFormatPreviewWithModule(format);
  const delim = format.delimiter || EPIC_DELIM;
  return `Naming rule for suggestedEpicTitle: {quarter}${delim}{system}${delim}{subsystem}${delim}[{module}${delim}]{capability} (use en-dash –; omit empty module). Roadmap example: ${preview4}. Ambition-table example: ${preview5}`;
}
