/**
 * Current-sprint payload compute helpers — observed window, days meta, stories, burndown series, decision cockpit.
 * SSOT for pure compute* functions used by lib/currentSprint.js buildCurrentSprintPayload.
 */

import { calculateWorkDays } from './kpiCalculations.js';
import { classifyIssueTypeForSplit, isWorkItemIssue, isSubtaskIssue } from './Delivera-Data-IssueType-Classification.js';
...
export function toHoursFromSeconds(seconds) {
  if (seconds == null || Number.isNaN(seconds)) return 0;
  return Math.round((seconds / 3600) * 10) / 10;
}

export function computeHoursSinceIso(iso, nowMs = Date.now()) {
  if (!iso) return null;
 ...
 */
export function computeObservedWorkWindow(issues, sprintStartDate, sprintEndDate) {
 ...
 */
export function computeFlags(observedWindow, plannedStart, plannedEnd) {
 ...
 */
export function computeDaysMeta(sprint, now = new Date()) {
 ...
 */
export function computeDailyCompletions(issues, storyPointsFieldId) {
...
export function computeStoriesList(issues, storyPointsFieldId, epicLinkFieldId) {
  const spField = storyPointsFieldId || '';
...
export function computeSubtaskTracking(issues) {
  const subtasks = [];
 ...
 */
export function computeRemainingWorkByDay(issues, sprintStartDate, sprintEndDate, storyPointsFieldId) {
 ...
 */
export function computeScopeChanges(issues, sprintStartDate, storyPointsFieldId) {
...
export function getCurrentBurndownIndex(remainingWorkByDay) {
...
export function buildSparklineFromSeries(series, limit = 6) {
...
export function buildDecisionCockpit({
  ...
  const primaryStuck = (Array.isArray(stuckCandidates) ? stuckCandidates : [])[0] || null;
  const nextBestActionRisk = primaryStuck
    ? (topRisks.find((r) => r.issueKey === primaryStuck.issueKey) || {
        issueKey: primaryStuck.issueKey,
        summary: primaryStuck.summary || 'Sprint item',
        status: primaryStuck.status || 'In Progress',
        assignee: primaryStuck.assignee || '',
        issueUrl: primaryStuck.issueUrl || buildIssueUrl(primaryStuck.issueKey),
        severity: 'High',
        reason: `The item has been stale for ${roundOne(primaryStuck.hoursInStatus || 24)}h and is likely blocking sprint flow.`,
        tags: ['No movement 24h'],
        riskTags: ['blocker'],
      })
    : (topRisks[0] || null);
  let orderedTopRisks = topRisks;
  if (primaryStuck?.issueKey) {
    const stuckKey = String(primaryStuck.issueKey).toUpperCase();
    const match = topRisks.find((r) => String(r.issueKey).toUpperCase() === stuckKey);
    if (match) {
      orderedTopRisks = [match, ...topRisks.filter((r) => String(r.issueKey).toUpperCase() !== stuckKey)];
    } else if (nextBestActionRisk) {
      orderedTopRisks = [nextBestActionRisk, ...topRisks].slice(0, 3);
    }
  }
  ...
  const nextBestAction = nextBestActionRisk
    ? {
        ...
        ctaLabel: primaryStuck ? 'Unblock' : 'Take Action',
