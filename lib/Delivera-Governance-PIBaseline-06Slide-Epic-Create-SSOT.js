...
import { playbookForProjects } from './Delivera-Governance-PIBaseline-05Slide-Playbook-01SSOT.js';
import { searchProjectBySummary } from './Delivera-Jira-Search-01SSOT.js';
...
async function findEpicByExactSummary(version3Client, projectKey, summary) {
  ...
  const issues = await searchProjectBySummary(version3Client, projectKey, escaped, { maxResults: 8 });
...
export async function reconcileSlideEpics({
  ...
  const qKey = quarterKey(quarter) || quarterKey(resolved[0]?.suggestedEpicTitle || '') || 'FY27 Q2';
  ...
  const nextResolved = reconcileResolvedWithEpics(resolved, jiraEpics, boardEpics, { playbook });
