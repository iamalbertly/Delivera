...
/** Boost similarity when both strings share technical acronyms (DMS, MIS, AMS, CSS, etc.) */
export function acronymBoost(a, b) {
  const acrRe = /\b[A-Z]{2,5}\b/g;
  const aAcr = new Set(String(a || '').match(acrRe) || []);
  const bAcr = new Set(String(b || '').match(acrRe) || []);
  if (!aAcr.size || !bAcr.size) return 0;
  const inter = [...aAcr].filter((x) => bAcr.has(x)).length;
  const union = new Set([...aAcr, ...bAcr]).size;
  return union > 0 ? inter / union : 0;
}

export function combinedTextSimilarity(a, b) {
  const base = Math.max(jaccardSimilarity(a, b), diceCoefficientBigrams(a, b));
  const acr = acronymBoost(a, b);
  // When both strings share acronyms, pull the score toward the acronym-match signal
  return acr > base ? (base * 0.5 + acr * 0.5) : base;
}
