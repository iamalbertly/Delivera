...
} from './Delivera-CurrentSprint-Value-Flow-Policy-SSOT.js';
...
function swarmFor(story, daysRemaining) {
  ...
  const stalled = open.filter((item) => n(item?.hoursInStatus) >= 24);
  const target = ready[0] || active.sort((a, b) => n(b?.hoursInStatus) - n(a?.hoursInStatus))[0];
  if (!target && !stalled.length) return null;
  const deadline = daysRemaining != null && daysRemaining <= 1 ? 'before today closes' : 'before the next stand-up';
  const parentInProgress = statusCategory(story) === 'indeterminate';
  const parentQuiet = n(story?.hoursInStatus ?? story?.ageHours) >= 24;
  const preferDoneProbe = parentInProgress && parentQuiet && stalled.length > 0;
  const assigneeFirstName = text(story?.assignee).split(/\s+/)[0] || '';
  return {
    ...
    stalledSubtasks: stalled.slice(0, 3).map((item) => ({
      issueKey: text(item?.issueKey).toUpperCase(),
      summary: text(item?.summary, 80),
      hoursInStatus: n(item?.hoursInStatus),
    })),
    preferDoneProbe,
    assigneeFirstName,
    question: preferDoneProbe
      ? `Can ${text(story?.issueKey).toUpperCase()} move to Done while ${stalled.slice(0, 3).map((item) => text(item?.issueKey).toUpperCase()).filter(Boolean).join(', ')} remain stalled?`
      : ready.length
        ? `Who has capacity to swarm ${target.issueKey} (${target.summary || 'the ready subtask'}) ${deadline}?`
        : `What does the squad need to swarm ${target?.issueKey || text(story?.issueKey)} and restore flow ${deadline}?`,
  };
}
...
export function enhanceFlowIntervention({
  ...
  commitments = [], daysRemaining = null, observedAt = '', context = {}, sprint = {},
  partialPermissions = false, stale = false,
} = {}) {
  ...
  const ranked = active.map((story) => {
    ...
    const dependencyRows = buildDependencyEvidence(story, commitment);
    const dependencies = dependencyRows.map((row) => row.issueKey);
    ...
    const interventionType = swarmPlan?.preferDoneProbe ? 'done-probe'
      : swarmPlan ? 'swarm-blocked-work'
        : dependencies.length ? 'protect-dependency'
          : wipPressure ? 'reduce-wip'
            : !commitment || !text(commitment?.piObjectiveTitle) ? 'confirm-pi-metadata' : 'restore-acceptance-proof';
    const flowEvidence = {
    ...
    const valueEvidence = {
      ...
      commitmentIssueKey: text(commitment?.issueKey).toUpperCase(),
      commitmentTitle: text(commitment?.title || commitment?.originalText),
      mappingState: commitment ? 'approved' : 'unmapped',
      mappingProvenance: commitment ? 'approved-baseline' : 'none',
    };
    const humanImpact = buildHumanImpact({ valueEvidence, story });
    const impact = humanImpact.statement || valueEvidence.businessValue || valueEvidence.businessCase
      || (dependencies.length ? `${dependencies.join(', ')} may be waiting on this work.`
        : commitment ? `Approved PI commitment ${valueEvidence.commitmentIssueKey || 'mapping'}: ${valueEvidence.commitmentTitle || story.summary || key}.`
          : 'No approved PI value mapping is available.');
    const ask = swarmPlan?.question || (dependencies.length
      ...
      : wipPressure
        ...
        : commitment
          ? `Keep ${key} moving under approved PI epic ${valueEvidence.commitmentIssueKey}; add the objective title for sponsor-ready traceability.`
    ...
    const reason = p85 != null && itemAge > p85
      ...
      : flowBaseline?.state === 'forming'
        ...
        : !commitment
          ...
          : !valueEvidence.piObjectiveTitle
            ? `${key} is mapped to approved PI epic ${valueEvidence.commitmentIssueKey}; the objective title is the remaining governance gap.`
          : `${key} is ranked by approved PI value and current flow evidence.`;
    const businessTime = buildBusinessTime({
      currentAgeHours: itemAge, p85CycleHours: p85, baselineState: flowBaseline?.state,
      partialPermissions, stale,
    });
    const strategicAnchor = buildStrategicAnchor({
      sprintName: sprint?.name, squadKey: context?.squadKey || context?.squadId,
      valueEvidence,
    });
    const impactScenario = buildImpactScenario({
      flowBaseline, daysRemaining, currentWip: statusCounts[status],
      partialPermissions, stale, contextConflict: strategicAnchor.conflict,
    });
    const communicationGuard = buildCommunicationGuard({
      stale, contextConflict: strategicAnchor.conflict, sendAllowed: !partialPermissions,
    });
    const secondaryRisk = dependencyRows.find((row) => row.direction === 'blocks' && row.squadKey
      && row.squadKey !== strategicAnchor.canonicalSquad) || null;
    const doneProbe = swarmPlan?.preferDoneProbe
      ? {
        prefer: true,
        assigneeFirstName: swarmPlan.assigneeFirstName || '',
        stalledSubtasks: swarmPlan.stalledSubtasks || [],
      }
      : null;
    return {
      ...
      ctaLabel: doneProbe ? 'Ask about Done' : swarmPlan ? 'Review swarm' : 'Review evidence',
      ...
      dependencyEvidence: { issueKeys: dependencies, rows: dependencyRows, source: dependencies.length ? 'approved-baseline-or-jira-link' : 'none' },
      swarmPlan, doneProbe, diagnosisConfidence: commitment || dependencies.length ? 0.92 : 0.74,
      humanImpact, businessTime, strategicAnchor, impactScenario, communicationGuard,
      secondaryRisk: secondaryRisk ? {
        issueKey: secondaryRisk.issueKey,
        squadKey: secondaryRisk.squadKey,
        consequence: `Verified dependency ${secondaryRisk.issueKey} may be affected.`,
      } : null,
  ...
  const nextBestAction = selected ? {
    ...
    interventionHash: hash({
      key: selected.issueKey, type: selected.interventionType, flow: selected.flowEvidence,
      value: selected.valueEvidence, dependency: selected.dependencyEvidence,
      context: { squad: context?.squadKey || context?.squadId, sprint: sprint?.id, truth: context?.truthHash },
    }),
