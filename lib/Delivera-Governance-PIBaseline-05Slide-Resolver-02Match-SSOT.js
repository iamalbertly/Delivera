...
function capabilitySegment(title = '') {
  const parts = normalizeEpicTitle(title).split(' - ');
  return parts[parts.length - 1] || normalizeEpicTitle(title);
}
...
function findBestEpicMatch(suggestedTitle, epics = [], minScore = SEMANTIC_MATCH_THRESHOLD, bullet = '') {
  ...
  const bulletNorm = normalizeEpicTitle(bullet || '');
  for (const e of epics) {
    ...
    const score = Math.max(
      ...
      bulletNorm ? titleSimilarity(bulletNorm, title) : 0,
    );
    if (score > bestScore) {
      bestScore = score;
      best = e;
  ...
  const effectiveMin = bulletNorm && capSuggested ? Math.min(minScore, 0.45) : minScore;
...
export function resolveSlideCommitments({
  ...
  for (const row of extracted) {
    ...
    const match = findBestEpicMatch(suggestedEpicTitle, allEpics, SEMANTIC_MATCH_THRESHOLD, row.bullet || row.title || '');
    let dupRisk = !match ? findDuplicateRisk(suggestedEpicTitle, allEpics, entry) : null;
    if (!match && dupRisk?.similarity >= 0.55 && dupRisk.issueKey) {
