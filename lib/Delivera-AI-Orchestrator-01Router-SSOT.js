...
function dedupeAiOutputForTask(output = {}, taskType) {
  ...
  if (taskType === AI_TASK_TYPES.PI_SLIDE_VISION && Array.isArray(next.commitments)) {
    next.commitments = uniqueBy(next.commitments.map((row) => {
      const deliveryPlan = Array.isArray(row?.deliveryPlan)
        ? row.deliveryPlan.map((d) => (typeof d === 'string' ? d : (d?.item || d?.name || d?.bullet || ''))).map((s) => String(s || '').trim()).filter(Boolean)
        : [];
      return {
        month: String(row?.month || row?.timeline || '').trim(),
        theme: String(row?.theme || row?.category || '').trim(),
        module: String(row?.module || '').trim(),
        deliveryItem: String(row?.deliveryItem || row?.delivery || '').trim(),
        deliveryPlan,
        ragStatus: String(row?.ragStatus || row?.status || row?.rag || '').trim(),
    ...
    }).filter((row) => row.bullet || row.title || row.deliveryItem || row.deliveryPlan.length), (row) => {
      return `${normalizeTextKey(row.month)}|${normalizeTextKey(row.module)}|${normalizeTextKey(row.theme)}|${normalizeTextKey(row.bullet || row.title || row.deliveryItem)}|${normalizeTextKey(row.deliveryPlan.join(','))}`;
    }).slice(0, 50);
    next.layout = String(next.layout || next.slideLayout || 'roadmap').trim().toLowerCase();
    next.squad = String(next.squad || '').trim();
    next.squadNickname = String(next.squadNickname || next.nickname || '').trim();
    next.program = String(next.program || next.projectName || next.tower || '').trim();
