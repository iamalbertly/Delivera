...
export function diagnosePromiseEvidence(input = {}) {
  ...
  if (input.boardResolved === false && !input.currentFound) {
  ...
  if (input.missingPiMetadata === true) {
    const jiraOnly = input.existsInJira === true && input.lifecycle === 'jira-only';
    return result(PROMISE_DIAGNOSIS_CODES.MISSING_PI_METADATA, jiraOnly
      ? 'Jira commitment exists but lacks PI metadata'
      : 'Active work lacks PI metadata', 0.92, [
      evidence('Jira key', issueKey),
      jiraOnly ? evidence('Jira status', status || 'Issue exists') : null,
      evidence('Missing fields', 'No FY/Qn in epic title, fiscal-quarter label, or fix version'),
    ], `Confirm the FY/Qn in the epic title (FY27 Q2 – Squad – Platform – Title) or add Fix Version for ${issueKey || 'this Jira item'}.`, jiraOnly
      ? 'The Jira item exists, but Delivera cannot connect it to current PI execution until its period is confirmed.'
      : 'Active delivery is invisible to PI reporting.');
  }
  ...
  if (!input.currentFound) {
    const notFound = input.notFoundInJira === true || Number(input.httpStatus) === 404;
    return result(PROMISE_DIAGNOSIS_CODES.EXACT_KEY_UNAVAILABLE, notFound
      ? 'Approved Jira key does not resolve'
      : 'Exact Jira key is not currently verifiable', notFound ? 0.96 : 0.70, [
      evidence('Requested key', issueKey),
      notFound
        ? evidence('Direct Jira result', '404; the approved key was not found')
        : evidence('Search scope', input.searchScope || 'Current authorized Jira evidence'),
    ], notFound
      ? `Confirm whether ${issueKey || 'this commitment'} moved, was re-keyed, or should be corrected in the approved baseline.`
      : `Check access, archive, rename, and board scope for ${issueKey || 'this commitment'}.`, notFound
      ? 'The approved commitment points to a Jira identity that no longer resolves; Delivera will not assume the work was removed.'
      : 'No delivery conclusion is safe until the source is resolved.');
  }
  const epicChildMatch = input.matchMethod === 'epic-child';
  return result(PROMISE_DIAGNOSIS_CODES.VERIFIED, epicChildMatch ? 'PI epic has active sprint stories' : 'Jira evidence verified', 0.99, [
    evidence('Jira key', issueKey),
    evidence('Jira status', status),
    epicChildMatch ? evidence('Current sprint stories', (input.matchedIssueKeys || []).join(', ')) : null,
    epicChildMatch ? evidence('Mapping method', 'Story → approved PI epic') : null,
    evidence('Sprint', input.sprintName),
