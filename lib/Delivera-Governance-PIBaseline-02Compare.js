...
export function diagnosePromiseEvidence(input = {}) {
  ...
  if (input.permissionDenied || input.httpStatus === 403 || input.boardResolved === false) {
    return result(PROMISE_DIAGNOSIS_CODES.ACCESS_BLOCKED, 'Jira access prevents verification', 0.98, [
      evidence('Requested commitment', issueKey),
      evidence('Access result', input.httpStatus === 403 ? 'Jira returned 403' : 'Configured board is not accessible'),
    ], `Restore Jira access for ${issueKey || 'this commitment'}, then recheck proof.`, 'Delivery may exist, but portfolio evidence is unavailable.');
  }
