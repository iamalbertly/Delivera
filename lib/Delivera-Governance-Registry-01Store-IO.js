...
export async function updateGovernanceRegistryBatch({ changes = [], reason = '', actor = 'unknown', idempotencyKey = '', expectedRegistryVersion = null }) {
  ...
  const lease = await Promise.race([
    cache.claimLease('organization-write', 15000, { namespace: 'governanceRegistryLease' }),
    new Promise((resolve) => setTimeout(() => resolve({ acquired: true, token: `memory-fallback:${Date.now()}`, key: 'organization-write', namespace: 'governanceRegistryLease', timedOut: true }), 2500)),
  ]);
  if (!lease.acquired) {
  ...
  try {
    ...
    if (expectedRegistryVersion != null && Number(expectedRegistryVersion) !== Number(current.version || 1)) {
      const error = new Error('Organization policy changed while you were editing. Review the field-level changes before publishing.');
      error.code = 'REGISTRY_VERSION_CONFLICT';
      error.httpStatus = 412;
      error.currentVersion = Number(current.version || 1);
      error.expectedVersion = Number(expectedRegistryVersion);
      throw error;
    }
