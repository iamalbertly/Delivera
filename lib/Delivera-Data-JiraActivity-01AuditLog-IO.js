...
export async function appendJiraActivityEntry(entry) {
  const row = {
    ...
    squadKey: String(entry?.squadKey || '').toUpperCase(),
    interventionHash: String(entry?.interventionHash || ''),
    truthHash: String(entry?.truthHash || ''),
    registryRevision: entry?.registryRevision == null ? null : Number(entry.registryRevision),
    validationRunId: String(entry?.validationRunId || ''),
    commitSha: String(entry?.commitSha || ''),
    cleanupRequired: Boolean(entry?.cleanupRequired),
    cleanupStatus: String(entry?.cleanupStatus || (entry?.cleanupRequired ? 'pending' : 'not-required')),
