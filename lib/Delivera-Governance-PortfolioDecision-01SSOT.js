  ...
  for (const entry of (Array.isArray(brief._boardPayloads) ? brief._boardPayloads : [])) {
    ...
    for (const story of (Array.isArray(entry?.payload?.stories) ? entry.payload.stories : [])) {
      ...
      addEpicLineageCandidate(candidates, {
        ...
        epicTitle: story.epicTitle || story.epicSummary,
        projectKey,
        storyCount: 1,
        status: story.status,
      }, anchorKey);
  ...
  return [
    {
      id: 'keep-funding',
      label: 'Keep funding',
      hint: 'Continue as planned',
      useWhen: 'Scope is confirmed and recovery remains feasible.',
      effect: 'No change to capacity.',
      impactPreview: `Maintains current allocation for ${name}. Use when proof confidence improves or commitments are confirmed.`,
      selected: defaultId === 'keep-funding',
    },
    {
      id: 'review-investment',
      label: 'Review investment',
      hint: 'Fix issues and revalidate outcomes',
      useWhen: 'Commitments remain unclear or evidence stays weak.',
      effect: 'Pause new allocation and validate outcomes.',
      impactPreview: `Pauses investment review for ${name} until ${exposed} exposed commitment${exposed === 1 ? '' : 's'} and ${nudges} prepared action${nudges === 1 ? '' : 's'} are resolved.`,
      selected: defaultId === 'review-investment',
    },
    {
      id: 'move-capacity',
      label: 'Move capacity',
      hint: 'Reallocate to higher impact',
...
export function buildPortfolioDecision({
  ...
  const periodKey = brief.meta?.quarter || '';
