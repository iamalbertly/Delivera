...
export function resolveBaselineMissingFromBrief(brief = {}, baselineMode = 'pi-baseline') {
  ...
  if (readinessKeys.length) {
    const anyReady = readinessKeys.some((k) => readiness[k]?.hasBaseline);
    if (anyReady) {
      // Portfolio-global flag is only true when *every* selected project lacks a baseline.
      const projects = (brief.projects || readinessKeys).map((p) => String(p).toUpperCase());
      return projects.every((pk) => !readiness[pk]?.hasBaseline);
    }
  }
  const byProject = brief?.baselineComparisonByProject || {};
  const byKeys = Object.keys(byProject);
  if (byKeys.length) {
    return byKeys.every((k) => !byProject[k]?.items?.length && !byProject[k]?.summary);
  }
...
function issueProjectKeyFromBrief(key = '') {
...
function legacyBaselineItemsForProject(brief = {}, projectKey = '') {
  ...
  return items.filter((item) => {
    ...
    const issuePk = issueProjectKeyFromBrief(item.issueKey);
    return squad === pk || issuePk === pk;
  });
}

/**
 * Per-project baseline readiness. Prefer meta.baselineReadinessByProject from assemble.
 * @returns {{ [projectKey: string]: { missing: boolean, hasBaseline: boolean, piName: string, baselineDate: string, committedCount: number } }}
 */
export function resolveBaselineReadinessByProject(brief = {}, projects = [], baselineMode = 'pi-baseline') {
  const pks = (projects.length ? projects : (brief.projects || []))
    .map((p) => String(p || '').trim().toUpperCase())
    .filter(Boolean);
  const metaReady = brief?.meta?.baselineReadinessByProject || {};
  const byCompare = brief?.baselineComparisonByProject || {};
  const out = {};
  for (const pk of pks) {
    const fromMeta = metaReady[pk];
    if (fromMeta) {
      out[pk] = {
        missing: !fromMeta.hasBaseline || baselineMode === 'none',
        hasBaseline: Boolean(fromMeta.hasBaseline) && baselineMode !== 'none',
        piName: fromMeta.piName || '',
        baselineDate: fromMeta.baselineDate || '',
        committedCount: Number(fromMeta.committedCount) || 0,
      };
      continue;
    }
    const compare = byCompare[pk];
    const legacyItems = legacyBaselineItemsForProject(brief, pk);
    const hasCompare = Boolean(compare?.summary || compare?.items?.length || legacyItems.length);
    const piName = compare?.piName || brief.baselineComparison?.piName || '';
    const baselineDate = compare?.baselineDate || brief.baselineComparison?.baselineDate || '';
    const committedCount = Number(compare?.summary?.totalCommitted)
      || (compare?.items?.length || 0)
      || legacyItems.length;
    out[pk] = {
      missing: !hasCompare || baselineMode === 'none',
      hasBaseline: hasCompare && baselineMode !== 'none',
      piName,
      baselineDate,
      committedCount,
    };
...
/** Whether a single project is missing its own PI baseline. */
export function isBaselineMissingForProject(brief = {}, projectKey = '', baselineMode = 'pi-baseline') {
  const pk = String(projectKey || '').trim().toUpperCase();
  if (!pk) return resolveBaselineMissingFromBrief(brief, baselineMode);
  const map = resolveBaselineReadinessByProject(brief, [pk], baselineMode);
  return Boolean(map[pk]?.missing);
}
...
function proofConfidencePct(insight = {}, brief = {}) {
  // Prefer countable evidence when present — never invent a high "confidence" for UI.
  const rows = brief?.evidencePack?.rows;
  if (Array.isArray(rows) && rows.length > 0) {
    const risks = Math.max(1, asNum(insight.cardRisks?.length, rows.length));
    const withProof = rows.filter((r) => r.hasEvidence || r.proofState === 'proven' || r.status === 'proven').length;
    return Math.max(8, Math.min(95, Math.round((withProof / risks) * 100)));
  }
  // No evidence pack: return null-ish 0 for UI consumers to treat as unavailable.
  if (!insight.boardResolved) return 0;
  const freshness = String(brief?.freshness?.confidenceLimit || 'live').toLowerCase();
  let base = 0;
  if (insight.verdictTier === 'blocked') base = 20;
  else if (insight.verdictTier === 'watch') base = 35;
  else if (insight.verdictTier === 'onTrack') base = 45;
  if (freshness === 'stale') base = Math.min(base, 25);
  return Math.max(0, Math.min(95, base));
}

/** User-facing proof label — never show synthetic high % without evidence rows. */
export function formatProofForUi(insight = {}, brief = {}) {
  const rows = brief?.evidencePack?.rows;
  if (Array.isArray(rows) && rows.length > 0) {
    const withProof = rows.filter((r) => r.hasEvidence || r.proofState === 'proven' || r.status === 'proven').length;
    return `${withProof} of ${rows.length} evidence rows`;
  }
...
/**
 * Resolve a squad's display name from the org catalog SSOT.
 * NEVER trust a Jira boardName over the catalog — a shared/foreign board
 * (e.g. "COPS PROJECT" for a DevSecOps board touching SD issues) must not
 * rename the squad. Catalog label → projectKey → boardName fallback.
 * (Audit 2026-07-15: "COPS PROJECT" phantom squad bug.)
 */
function squadDisplayName(insight = {}) {
  const pk = String(insight.projectKey || '').trim().toUpperCase();
  if (pk) {
    const entry = catalogEntry(pk);
    if (entry?.label) return entry.label;
  }
  return pk || insight.boardName || 'Squad';
}
...
function buildSquadScope() {
  const deliveryKeys = deliverySquadKeys();
  const operationalKeys = operationalEntityKeys();
  return {
    scoreableSquadCount: deliveryKeys.length,
    deliverySquadCount: deliveryKeys.length,
    operationalEntityCount: operationalKeys.length,
    excludedEntityKeys: operationalKeys,
    label: `${deliveryKeys.length} delivery squads; ${operationalKeys.length} operational guild excluded`,
  };
}

function readinessHasBaseline(entry = {}) {
  return Boolean(entry?.hasBaseline) && !entry?.missing;
}

function buildBaselineCoverage({ readiness = {}, periodKey = '', timebox = {} } = {}) {
  const deliveryKeys = deliverySquadKeys();
  const rows = deliveryKeys.map((key) => ({
    projectKey: key,
    hasBaseline: readinessHasBaseline(readiness[key]),
    committedCount: Number(readiness[key]?.committedCount) || 0,
    piName: readiness[key]?.piName || '',
  }));
  const uploaded = rows.filter((row) => row.hasBaseline).length;
  const missing = rows.length - uploaded;
  const elapsedRatio = timebox.totalDays > 0 ? timebox.elapsedDays / timebox.totalDays : 0;
  const requestNextQuarter = elapsedRatio >= 0.8;
  return {
    periodKey,
    totalSquads: rows.length,
    uploaded,
    missing,
    missingProjectKeys: rows.filter((row) => !row.hasBaseline).map((row) => row.projectKey),
    currentQuarterLine: `${missing} of ${rows.length} delivery squads do not have uploaded PI commitments for ${periodKey || 'the selected quarter'}`,
    nextQuarterRequest: {
      active: requestNextQuarter,
      threshold: '80%',
      reason: requestNextQuarter
        ? 'Quarter is inside the next-PI preparation window'
        : 'Next-quarter PI request starts when the quarter reaches 80% elapsed',
    },
  };
}

function buildAttentionBreakdown({ judgment = {}, baselineCoverage = {} } = {}) {
  const squads = Array.isArray(judgment.squads) ? judgment.squads : [];
  const deliveryRisk = squads.filter((s) =>
    !s.baselineMissing
    && (s.attentionState === 'off-plan' || s.attentionState === 'decision-required')
  ).length;
  const missingStoryEvidence = squads.filter((s) => !s.baselineMissing && Number(s.notPlannedCount) > 0).length;
  const cannotJudge = squads.filter((s) => s.baselineMissing || s.dataTrust === 'board-health-only' || !s.boardResolved).length;
  return {
    deliveryRisk,
    missingBaseline: Number(baselineCoverage.missing) || 0,
    missingStoryEvidence,
    cannotJudge,
    line: `${deliveryRisk} delivery risk; ${baselineCoverage.missing || 0} squads missing PI commitments; ${missingStoryEvidence} squads missing Jira story evidence`,
  };
}
...
export function buildPortfolioDecision({
  ...
  const rawAnchor = String(anchorProject || '').trim().toUpperCase();
  const portfolioGeneral = rawAnchor === '__ALL__';
  const anchorKey = portfolioGeneral
    ? String(projects[0] || insights[0]?.projectKey || '').trim().toUpperCase()
    : (rawAnchor || String(projects[0] || '').trim().toUpperCase());
  const compareKeys = (portfolioGeneral
    ? (compareProjects.length ? compareProjects : projects)
    : (compareProjects.length ? compareProjects : projects.filter((p) => p !== anchorKey)))
    .map((p) => String(p).trim().toUpperCase())
    .filter((p) => p && p !== '__ALL__');
  const anchor = insights.find((i) => i.projectKey === anchorKey) || insights[0] || { projectKey: anchorKey };
  const peers = insights.filter((i) => compareKeys.includes(i.projectKey) && i.projectKey !== anchorKey);
  const periodKey = String(periodKeyArg || brief.meta?.quarter || '').trim();
  const effectiveAnchorMissing = portfolioGeneral
    ? false
    : (brief?.meta?.baselineReadinessByProject?.[anchorKey]
      ? Boolean(!brief.meta.baselineReadinessByProject[anchorKey].hasBaseline)
      : isBaselineMissingForProject(brief, anchorKey, baselineMode) || Boolean(baselineMissing && !brief?.baselineComparisonByProject?.[anchorKey]));
  const recommendation = resolveRecommendation(anchor, peers, cases, effectiveAnchorMissing, brief);
  const gap = classifyPortfolioGap({ anchor, peers, cases, brief, baselineMissing: effectiveAnchorMissing });
  ...
  const portfolioSummary = buildPortfolioSummary({
    ...
    cases,
    epicLineage,
    baselineMissing: effectiveAnchorMissing,
    partialSquads,
  });
  const dataTrust = buildDataTrust({ brief, insights, epicLineage, baselineMissing: effectiveAnchorMissing, partialSquads });
  ...
  const squadScope = buildSquadScope();
  const baselineReadinessByProject = resolveBaselineReadinessByProject(
    brief,
    projects.length ? projects : insights.map((i) => i.projectKey),
    baselineMode,
  );
  const baselineCoverage = buildBaselineCoverage({
    readiness: baselineReadinessByProject,
    periodKey,
    timebox,
  });
  const attentionBreakdown = buildAttentionBreakdown({
    judgment: portfolioJudgment,
    baselineCoverage,
  });
  const priorityBrief = buildPriorityBrief({
    brief,
    decision: {
      anchorProject: portfolioGeneral ? '__ALL__' : (anchor.projectKey || anchorKey),
      portfolioGeneral,
      periodKey,
      insights,
      epicLineage,
  ...
  return {
    ...
    baselineCoverage,
    attentionBreakdown,
    comparisonReason: portfolioJudgment.squads.map((s) => ({
