...
export async function runProposePipeline({
  ...
  if (cache && candidates.length) {
    const activity = await loadEpicActivityFromBriefCache({ projects: pks, cache, namespace: GOVERNANCE_NS });
    candidates = enrichCandidatesWithEpicActivity(candidates, activity);
  }

  const boardEpicCount = candidates.filter((c) => c.method?.includes('board')).length;
  let guidance = null;
  if (!candidates.length) {
    guidance = 'No epics on the board yet. Use Create work to add them first.';
  } else if (boardEpicCount === 0) {
    guidance = 'Epics found in Jira — none auto-matched. Upload your PI plan or pick manually below.';
  }
