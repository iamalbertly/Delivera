...
export async function callOpenRouterVision(system, userText, imageBase64, mimeType, apiKey, model = 'google/gemini-2.5-flash-lite', options = {}) {
  ...
  if (options.cacheEnabled) {
    ...
    headers['X-OpenRouter-Cache-TTL'] = String(Math.min(86400, Number(options.cacheTtlSeconds) || 86400));
  }
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    signal: options.signal,
    ...
    body: JSON.stringify({
      ...
      provider: { zdr: options.zdr !== false },
      response_format: { type: 'json_object' },
      messages: [
  ...
  return {
    ...
    model: data?.model || model,
    cacheStatus: res.headers.get('x-openrouter-cache') || '',
  };
}
...
export async function runVisionAITask(taskContract, payload, providerConfig, budgetMeta) {
  ...
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), budgetMeta?.timeoutMs || 90000);
  try {
    let response = { text: '', usage: {}, model: providerConfig.model, cacheStatus: '' };
    if (provider === 'openai') {
      response.text = await callOpenAiVision(system, userText, imageBase64, mimeType, apiKey);
    } else if (provider === 'claude') {
      response.text = await callClaudeVision(system, userText, imageBase64, mimeType, apiKey);
    } else if (provider === 'openrouter') {
      response = await callOpenRouterVision(system, userText, imageBase64, mimeType, apiKey, providerConfig.model, {
        maxTokens: budgetMeta?.maxTokens,
        temperature: 0.1,
        signal: controller.signal,
        zdr: true,
        cacheEnabled: providerConfig.cacheEnabled === true,
    ...
    return {
      parsed: parseTaskResponse(response.text, taskContract),
