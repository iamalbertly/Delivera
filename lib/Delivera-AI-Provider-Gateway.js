 ...
 * Env (.env) is default; browser override wins when client sends confirmed headers (x-ai-override + key).
 *
 * @param {object} reqHeaders  - Request headers object (lowercased keys)
 * @returns {{ provider: string, apiKey: string, host: string, source: string }}
 */
export function resolveProviderConfig(reqHeaders = {}) {
  ...
  const headerOverride = String(reqHeaders['x-ai-override'] || '').trim() === '1';

  const envProvider = aiProviderEnvConfig.defaultProvider || 'built-in';
  const browserOverride = headerOverride && headerProvider && headerProvider !== 'built-in' && headerKey;

  const provider = browserOverride ? headerProvider : envProvider;
  ...
  switch (provider) {
    case 'claude':
      apiKey = browserOverride ? headerKey : (aiProviderEnvConfig.claudeApiKey || headerKey);
      break;
    case 'openai':
      apiKey = browserOverride ? headerKey : (aiProviderEnvConfig.openaiApiKey || headerKey);
      break;
    case 'gemini':
      apiKey = browserOverride ? headerKey : (aiProviderEnvConfig.geminiApiKey || headerKey);
      break;
    case 'openrouter':
      apiKey = browserOverride ? headerKey : (aiProviderEnvConfig.openrouterApiKey || headerKey);
      break;
    case 'ollama':
      host = browserOverride
        ? (headerHost || 'http://localhost:11434')
        : (aiProviderEnvConfig.ollamaHost || headerHost || 'http://localhost:11434');
      break;
    default:
      break;
  }

  if (browserOverride) source = 'browser';
  else if (provider !== 'built-in' && (apiKey || provider === 'ollama')) source = 'server';

  return { provider: provider || 'built-in', apiKey, host, source };
}
...
export async function callOpenRouterVision(system, userText, imageBase64, mimeType, apiKey, model = 'google/gemini-2.5-flash-lite', options = {}) {
  ...
  // Some OpenRouter models (e.g. Gemini Flash) don't support response_format json_object with vision.
  // Only include it for models known to support JSON mode with images.
  const supportsJsonMode = /gpt-4o|claude|sonnet/i.test(model || '');
  const body = {
    model: model || 'google/gemini-2.5-flash-lite',
    max_tokens: Number(options.maxTokens) || undefined,
  ...
  if (!res.ok) {
    ...
    let errMsg = `OpenRouter vision error ${res.status}`;
    try {
    ...
    console.warn('[callOpenRouterVision] error', { status: res.status, errMsg, model });
...
export async function callClaudeVision(system, userText, imageBase64, mimeType, apiKey, options = {}) {
  const media = String(mimeType || 'image/png').split(';')[0].trim() || 'image/png';
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: {
...
export async function callOpenAiVision(system, userText, imageBase64, mimeType, apiKey, options = {}) {
  const media = String(mimeType || 'image/png').split(';')[0].trim() || 'image/png';
  const dataUrl = `data:${media};base64,${imageBase64}`;
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    method: 'POST',
...
export async function runVisionAITask(taskContract, payload, providerConfig, budgetMeta) {
  ...
  const userText = payload.userText || buildUserPromptForTask(taskContract, payload);
  const system = payload.systemPrompt || buildSystemPromptForTask(taskContract);

  // AbortController timeout — vision calls can hang indefinitely without this.
  // Uses the budget guard's timeoutMs (default 60s for vision — images need more time than text).
  const timeoutMs = Number(budgetMeta?.timeoutMs) || 60000;
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  ...
  try {
    let rawText = '';
    if (provider === 'openai') {
      rawText = await callOpenAiVision(system, userText, imageBase64, mimeType, apiKey, { signal: controller.signal });
    } else if (provider === 'claude') {
      rawText = await callClaudeVision(system, userText, imageBase64, mimeType, apiKey, { signal: controller.signal });
    ...
    if (err?.name === 'AbortError') {
      throw new Error(`Slide vision timed out after ${timeoutMs / 1000}s — try a smaller image or retry.`);
    }
    throw err;
  } finally {
    clearTimeout(timer);
