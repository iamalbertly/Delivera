...
function contractTruth({ baselineCoverage, counts, attentionCount, piPct }) {
  ...
  if (attentionCount) return { state: 'cannot-verify', label: `${attentionCount} commitment${attentionCount === 1 ? '' : 's'} cannot be verified`, detail: 'Evidence requires PI Team review.' };
...
function trustFactorForSquad({ baselineCoverage, sprintReality, boardResolved, ownerRoute, proofState, operatingModel }) {
  ...
  if (/stale|review/.test(proofState || '')) gaps.push('proof gap');
...
export function clusterUnknownWork({ activeItems = [], workSplit = null, thresholdPercent = DOING_INSTEAD_POLICY.minimumPercent } = {}) {
  ...
  return {
    ...
    copy: unknownPct >= thresholdPercent && clusters[0]
      ? `Unknown work is ${unknownPct}%. Classify top cluster: ${clusters[0].title}.`
      : unknownPct > 0 ? `Unknown work is ${unknownPct}%. Kept visible without adding meeting noise.` : 'No Unknown work is present.',
  };
}
...
function sprintRealityForInsight(insight = {}) {
  ...
  const endedWithoutReplacement = Boolean(pulse.endedWithoutReplacement || insight.endedWithoutReplacement || (pulse.sprintEnded && !pulse.nextSprint && !pulse.activeSprint));
  ...
  const carryoverRecurrence = Math.max(0, Number(pulse.carryoverRecurrence || insight.carryoverRecurrence) || 0);
  const reopened = Math.max(0, Number(pulse.reopened || insight.reopenedCount) || 0);
  const completedAfterEnd = Math.max(0, Number(pulse.completedAfterEnd || insight.completedAfterSprintEnd) || 0);
  const parts = [endedWithoutReplacement ? 'Sprint ended and no active replacement sprint was detected.' : age ? `Sprint started ${age} days ago.` : 'Sprint is active.'];
  if (stalled) parts.push(`${stalled} PI-linked item${stalled === 1 ? ' has' : 's have'} not moved.`);
  if (carried) parts.push(`${carried} item${carried === 1 ? '' : 's'} carried over${carryoverRecurrence ? ` across ${carryoverRecurrence} sprint${carryoverRecurrence === 1 ? '' : 's'}` : ''}.`);
  if (completedAfterEnd) parts.push(`${completedAfterEnd} finished after sprint end.`);
  if (reopened) parts.push(`${reopened} reopened after Done.`);
  return { state: endedWithoutReplacement ? 'missing-next-sprint' : stalled || carried || reopened ? 'watch' : 'active', ageDays: age, stalled, carried, carryoverCount: carried, carryoverRecurrence, completedAfterEnd, reopened, endedWithoutReplacement, copy: parts.join(' ') };
}
...
export function buildActiveGovernanceAnswer({ brief = {}, baseline = null, caseState = {}, now = new Date() } = {}) {
  ...
  const promises = source.map((item, ordinal) => {
    ...
    const labels = { 'send-nudge': ownerRoute.unresolved ? 'Send to PI Team assignment queue' : `Nudge ${ownerRoute.role}: ${ownerRoute.displayName}`, 'pull-fresh-evidence': 'Pull fresh proof', 'amend-contract': 'Amend contract', 'approve-match': 'Approve match', 'assign-owner': 'Owner route missing · resolve in drawer', 'accept-risk': 'Accept risk' };
    ...
    promise.drawerStateHash = governancePayloadHash({
      promiseId, version: promise.version, matchState, caseState: promise.caseState,
  ...
  let squads = squadKeys.map((squad) => {
    ...
    const contractState = contractTruth({ baselineCoverage, counts, attentionCount: squadAttention, piPct });
    ...
    projected.payloadHash = governancePayloadHash({
      squad, version: projected.version, counts, proofState, workSplit, doingInstead,
      unknownWork, possibleRework, sprintReality: projected.sprintReality, baselineCoverage,
      displayName, contractState, trustFactor: projected.trustFactor, operatingModel,
  ...
  const answer = !baseline && !brief?.baselineComparison
    ...
    : !complete
      ...
      : needsAttention.length
        ? `${needsAttention.length} squad${needsAttention.length === 1 ? ' is' : 's are'} not aligned to PI promises. ${needsAttention.slice(0, 2).map((s) => `${s.displayName} has ${s.topState}`).join('. ')}.`
  ...
  const lensSummaries = {
    ...
    sprint: `${squads.filter((squad) => ['missing', 'missing-next-sprint'].includes(squad.sprintReality?.state)).length} squads have unverified sprint cadence. Prioritize those that also have PI contract gaps.`,
  ...
  return {
    ...
    freshness,
    loopCompletion,
    decisionCoverage: { closed: closedDecisions, total: totalChecked, preparedOwnerAsks: prepared, copy: `Decision coverage: ${closedDecisions} of ${totalChecked} gaps closed` },
    lensSummaries,
    squads,
