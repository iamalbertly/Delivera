...
export const GOVERNANCE_PRESENTATION_CONTRACT_VERSION = 3;
...
function contractTruth({ baselineCoverage, counts, attentionCount, piPct, evidenceComplete = true }) {
...
export function buildActiveGovernanceAnswer({ brief = {}, baseline = null, caseState = {}, now = new Date() } = {}) {
  ...
  let squads = squadKeys.map((squad) => {
    ...
    const evidenceComplete = insight.boardResolved !== false
      ...
      && !jiraFailed;
    const contractState = contractTruth({ baselineCoverage, counts, attentionCount: squadAttention, piPct, evidenceComplete });
