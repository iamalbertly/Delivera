...
export const GOVERNANCE_PRESENTATION_CONTRACT_VERSION = 5;
...
export function buildActiveGovernanceAnswer({ brief = {}, baseline = null, caseState = {}, now = new Date() } = {}) {
  ...
  const promises = source.map((item, ordinal) => {
    ...
    const relatedItems = activeItems.filter((entry) => {
      ...
      return key === issueKey || titleWords.filter((word) => title.includes(word)).length >= Math.min(2, titleWords.length);
    });
    const currentFound = Boolean(row.issueKey || currentEpicKeys.has(issueKey) || observed);
    const diagnosis = diagnosePromiseEvidence({
      ...
      missingPiMetadata: row.missingPiMetadata ?? (currentFound && !row.fixVersion && !row.quarterLabel && !row.piLabel),
      fixVersion: row.fixVersion,
      quarterLabel: row.quarterLabel || row.piLabel,
      baselinePeriod: fiscalPeriod,
      jiraPeriod: row.jiraPeriod || row.fixVersion || row.quarterLabel,
      candidateIssueKeys: relatedItems.map((entry) => entry.issueKey || entry.key).filter(Boolean),
      candidateReason: relatedItems.length ? 'Strong title overlap in the same canonical squad' : '',
      searchScope: insight.boardResolved === false ? 'Configured board could not be resolved' : 'Current canonical squad and board scope',
