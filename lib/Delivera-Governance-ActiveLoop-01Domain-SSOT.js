...
function friendlyWorkTheme(item = {}) {
  ...
  if (bracketed) return { title: clean(bracketed, 100), systemDerived: true };
...
export function buildActiveGovernanceAnswer({ brief = {}, baseline = null, caseState = {}, now = new Date() } = {}) {
  ...
  let squads = squadKeys.map((squad) => {
    ...
    const projected = {
      ...
      baselineCoverage,
      boardResolved: insight.boardResolved !== false,
      governanceInclusion: insight.operationalGroup === true ? 'excluded-operational-group' : 'pi-governed',
  ...
  squads = squads.sort((a, b) => b.riskScore - a.riskScore || a.squad.localeCompare(b.squad))
  ...
  const lensSummaries = {
    overall: needsAttention.length ? `${needsAttention.length} squad${needsAttention.length === 1 ? '' : 's'} need PI Team attention.` : 'No verified PI variance requires intervention.',
    rollover: `${squads.filter((squad) => Number(squad.sprintReality?.carryoverCount) > 0).length} squads show sprint carryover evidence.`,
    sprint: `${squads.filter((squad) => squad.sprintReality?.endedWithoutReplacement).length} squads have an ended sprint with no active replacement detected.`,
    operational: `${squads.filter((squad) => squad.doingInstead?.major).length} squads have a material Doing Instead diversion.`,
    unknown: `${squads.filter((squad) => squad.unknownWork?.promoted).length} squads have Unknown work above the cleanup threshold.`,
    rework: `${squads.filter((squad) => squad.possibleRework?.promoted?.confidence === 'high').length} squads have high-confidence possible rework evidence.`,
  };
  return {
