...
function freshnessShortLabel(freshness = {}) {
...
export function buildSetupGaps(brief = {}, opts = {}) {
  ...
  if (!hasBaseline) {
    gaps.push({
      id: 'pi-baseline',
      label: 'PI baseline missing — carryover and removed cannot be proven',
      action: 'set-baseline',
      severity: 'high',
    });
  }
...
export async function buildWorkerReceipt(brief = {}, inboxGrouped = {}, jobs = []) {
  ...
  return {
    line: authFailed
      ? `Last run: ${ago} · Jira auth failed — reconnect · Prepared: ${briefs} brief, ${nudges} nudges · Needs: reconnect`
      : `Last run: ${ago} · Checked: Jira, sprint, evidence · Prepared: ${briefs} brief, ${nudges} nudges${confirm ? `, ${confirm} review` : ''} · Needs: ${needs}`,
