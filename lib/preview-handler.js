...
import { filterSprintCapableBoards } from './Delivera-Data-Board-Sprint-Capability-01SSOT.js';
...
export async function previewHandler(req, res) {
    ...
    try {
        ...
        // Kanban/simple boards are excluded from sprint APIs (Jira returns 400).
        const sprintCapableBoards = filterSprintCapableBoards(boards);
        const sprintFetchFailures = [];
        const allSprints = [];
        for (let i = 0; i < sprintCapableBoards.length; i += 3) {
            if (cancelled) break;
            const chunk = sprintCapableBoards.slice(i, i + 3);
            const res = await Promise.allSettled(
                chunk.map(b =>
                    retryOnRateLimit(
                        () => fetchSprintsForBoard(b.id, agileClient, { board: b }),
                        3,
                        `sprints:${b.id}`
                    )
                )
        ...
        addPhase('fetchSprints', {
          count: allSprints.length,
          failedBoards: sprintFetchFailures.length,
          skippedNonScrum: boards.length - sprintCapableBoards.length,
        });
        if (!allSprints.length && sprintFetchFailures.length > 0 && sprintCapableBoards.length > 0) {
            throw new PreviewError(
                'BOARD_FETCH_ERROR',
                502,
        ...
        for (let i = 0; i < sprintIds.length; i += 3) {
            ...
            const res = await Promise.all(chunk.map(async (sid) => {
                ...
                try {
                    ...
                    sprintIssueFailures.push({
                        sprintId: sid,
                        sprintName: sprint?.name || '',
                        boardId: board?.id ?? null,
                        boardName: board?.name || '',
                        error: e?.message || 'Unknown sprint issue fetch error',
                    });
                    return [];
                }
            }));
        ...
        const meta = {
            ...
            phaseLog,
            fieldInventory,
            reducedScope: sprintFetchFailures.length > 0 || sprintIssueFailures.length > 0,
            reducedScopeReason: (sprintFetchFailures.length > 0 || sprintIssueFailures.length > 0)
                ? `${sprintFetchFailures.length} board(s) failed sprint retrieval; ${sprintIssueFailures.length} sprint(s) failed issue retrieval`
                : '',
            failedBoards: sprintFetchFailures.slice(0, 5),
            failedBoardCount: sprintFetchFailures.length,
            failedSprints: sprintIssueFailures.slice(0, 8),
            failedSprintCount: sprintIssueFailures.length,
        ...
        // Stale-on-error: when Jira is unreachable (502), serve the last good cached
        // response rather than an error page — users see slightly-old data with a banner
        // instead of a blank screen, which is almost always preferable during outages.
        const isJiraOutage = err instanceof PreviewError && err.httpStatus === 502;
        if (isJiraOutage && realCacheKey) {
            const staleEntry = await cache.getWithStaleFallback(realCacheKey).catch(() => null);
            if (staleEntry) {
                const val = staleEntry.value || staleEntry;
                logger.warn('Serving stale preview during Jira outage', {
                    key: realCacheKey,
                    staleAgeMs: staleEntry.staleAgeMs,
                    code: err.code,
                });
                return res.json({
                    ...val,
                    meta: {
                        ...
                        stale: true,
                        staleAgeMs: staleEntry.staleAgeMs,
                        staleReason: err.code || 'JIRA_UNREACHABLE',
                        jiraHost: jiraHostResolved,
                    },
                });
            }
