...
const FALLBACK_PROJECTS = ['MPSA', 'MAS'];
...
function riskToUseCase(riskType) {
  switch (String(riskType || '').toLowerCase()) {
    ...
    case 'dependency': return 'blocker';
...
async function resolveGovernanceProjects() {
  try {
    ...
    const merged = [...new Set([...accessible, ...snapshot, ...FALLBACK_PROJECTS])];
    return merged.length ? merged.slice(0, 12) : FALLBACK_PROJECTS;
  } catch (_) {
