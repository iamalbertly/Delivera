...
import { getLatestPIBaseline, migrateExistingPIBaselinesToDurableStore } from './Delivera-Governance-PIBaseline-01Store-IO.js';
...
import { governanceBriefCacheKey } from './Delivera-Governance-Story-Cache-01SSOT.js';
import { selectBoardsForSprintOps, filterSprintCapableBoards } from './Delivera-Data-Board-Sprint-Capability-01SSOT.js';
import { readGovernanceRegistry } from './Delivera-Governance-Registry-01Store-IO.js';

const GOVERNANCE_NS = 'governanceBrief';
const GOVERNANCE_BRIEF_TTL_MS = 30 * 60 * 1000;
const BUILT_IN_PROVIDER = { provider: 'built-in' };
const CHECK_INTERVAL_MS = 15 * 60 * 1000;
/** Defer first tick so HTTP + UI bind before cold Jira storm (mirrors snapshot 30s). */
export const GOVERNANCE_WORKER_STARTUP_DELAY_MS = Number(process.env.GOVERNANCE_WORKER_STARTUP_DELAY_MS || 45_000);

let governanceWorkerInFlight = false;
let governanceWorkersStartedAt = 0;
...
async function buildBriefForProjects(projects) {
  ...
  let boards = filterSprintCapableBoards(discoveredBoards);
  try {
...
async function runPrepareWeeklyBrief(projects) {
  if (await hasRecentRunningJob({ type: 'prepare-weekly-brief', projects, withinMinutes: 10 })) {
    ...
    const cacheKey = governanceBriefCacheKey(projects);
    const hit = await cache.get(cacheKey, { namespace: GOVERNANCE_NS });
...
async function governanceWorkerTick() {
  ...
  if (process.env.DELIVERA_SKIP_COLD_GOVERNANCE_TICK === '1' && !governanceWorkersStartedAt) return;
  ...
  try {
    ...
    // Single brief build per tick — prepare-weekly-brief owns inbox + cache.
    const brief = await runPrepareWeeklyBrief(projects);
...
export function getGovernanceWorkerStartupState() {
  const started = governanceWorkersStartedAt > 0;
  const graceMs = GOVERNANCE_WORKER_STARTUP_DELAY_MS;
  const elapsed = started ? Date.now() - governanceWorkersStartedAt : 0;
  return {
    workersStarted: started,
    startupGrace: started && elapsed < graceMs,
    startupDelayMs: graceMs,
  };
}

export function startGovernanceWorker() {
  void migrateExistingPIBaselinesToDurableStore().catch((err) => logger.warn('PI baseline durable migration failed', { error: err?.message }));
  governanceWorkersStartedAt = Date.now();
  const delay = process.env.DELIVERA_SKIP_COLD_GOVERNANCE_TICK === '1'
    ? Number.POSITIVE_INFINITY
    : GOVERNANCE_WORKER_STARTUP_DELAY_MS;
  if (Number.isFinite(delay)) {
    setTimeout(() => governanceWorkerTick(), Math.max(0, delay));
  }
  setInterval(() => governanceWorkerTick(), CHECK_INTERVAL_MS);
  logger.info('Governance worker scheduler started', { startupDelayMs: Number.isFinite(delay) ? delay : null });
}
