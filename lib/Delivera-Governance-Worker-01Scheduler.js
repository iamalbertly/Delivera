...
import { getLatestPIBaseline, listPIBaselines, migrateExistingPIBaselinesToDurableStore } from './Delivera-Governance-PIBaseline-01Store-IO.js';
...
import { buildActiveGovernanceAnswer, projectActiveGovernanceLayer1 } from './Delivera-Governance-ActiveLoop-01Domain-SSOT.js';
import { projectActiveLoopCases, readActiveLoopEvents } from './Delivera-Governance-ActiveLoop-02Store-IO.js';
import { GOVERNANCE_STORY_DETAIL_NAMESPACE, governanceBriefCacheKey, governanceStoryCacheKey, rememberWarmGovernanceStory } from './Delivera-Governance-Story-Cache-01SSOT.js';
...
async function buildBriefForProjects(projects) {
  ...
  const selected = projects.map((project) => rows.find((row) => (row.projects || []).includes(project))).filter(Boolean);
  const storyBaseline = selected.length ? {
    id: `portfolio:${selected.map((row) => row.id).sort().join('+')}`,
    piName: selected.map((row) => row.piName).filter(Boolean).join(' + '),
    projects,
    source: 'approved-portfolio-baselines',
    sourceBaselines: selected.map((row) => ({ ...row, committedItems: undefined })),
    committedItems: selected.flatMap((row) => row.committedItems || []),
  } : baseline;
  const events = await readActiveLoopEvents({ limit: 5000 });
  const fullStory = buildActiveGovernanceAnswer({ brief, baseline: storyBaseline, caseState: projectActiveLoopCases(events) });
  const story = projectActiveGovernanceLayer1(fullStory);
  await Promise.all([
    cache.set(governanceStoryCacheKey(projects), story, 60 * 60 * 1000, { namespace: 'governanceStoryV2' }),
    cache.set(governanceStoryCacheKey(projects), fullStory, 60 * 60 * 1000, { namespace: GOVERNANCE_STORY_DETAIL_NAMESPACE }),
  ]);
  rememberWarmGovernanceStory(governanceStoryCacheKey(projects), story);
  return brief;
}
...
export function startGovernanceWorker() {
  ...
  setTimeout(() => governanceWorkerTick(), 0);
