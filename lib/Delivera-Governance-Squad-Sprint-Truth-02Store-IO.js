...
/** Map a story/issue/subtask into the compact currentWork shape used by Active Loop matching. */
function mapWorkItem(item, payload = {}, overrides = {}) {
  const parentKey = clean(overrides.parentKey || item.parentKey || item.parentIssueKey || item.fields?.parent?.key);
  const epicKey = clean(overrides.epicKey || item.epicKey || item.epic?.key || item.fields?.epic?.key || parentKey);
  return {
...
/**
 * Stories first, then subtasks with parent→epic rollup so subtask-only delivery
 * still proves a PI epic via livePromiseMatch (epic-child / parentKey).
 */
function buildCurrentWorkFromPayload(payload = {}) {
  const stories = payload?.stories || payload?.issues || [];
  const storyByKey = new Map();
  const work = [];
  for (const item of stories) {
    const mapped = mapWorkItem(item, payload);
    if (!mapped.issueKey) continue;
    storyByKey.set(mapped.issueKey.toUpperCase(), mapped);
    work.push(mapped);
  }
  const subtasks = Array.isArray(payload?.subtasks)
    ? payload.subtasks
    : Array.isArray(payload?.subtaskTracking?.subtasks)
      ? payload.subtaskTracking.subtasks
      : [];
  for (const sub of subtasks) {
    const parentKey = clean(sub.parentKey || sub.parentIssueKey || sub.fields?.parent?.key);
    const parent = parentKey ? storyByKey.get(parentKey.toUpperCase()) : null;
    const mapped = mapWorkItem(sub, payload, {
      parentKey,
      epicKey: clean(sub.epicKey || parent?.epicKey || parentKey),
    });
    if (!mapped.issueKey) continue;
    if (work.some((row) => row.issueKey.toUpperCase() === mapped.issueKey.toUpperCase())) continue;
    work.push(mapped);
  }
  return work.slice(0, 80);
}
...
export async function writeSquadSprintTruth({ squadKey, quarter = 'current', payload, checkedBoards = [] } = {}) {
  ...
  const record = {
    ...
    currentWork: buildCurrentWorkFromPayload(payload),
  };
  if (previous) {
