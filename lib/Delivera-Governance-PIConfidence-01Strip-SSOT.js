...
export function buildPIConfidenceStrip(brief = {}, boardPayloads = []) {
  ...
  const timelineChips = items.slice(0, 8).map((item) => {
    ...
    const act = activityByKey.get(String(item.issueKey || '').toUpperCase()) || item.epicActivity || null;
    const storyCount = Number(act?.storyCount) || 0;
    const doneCount = Number(act?.doneCount) || 0;
    const deliveryPct = verdict === BASELINE_VERDICTS.DELIVERED
      ? 100
      : (storyCount ? Math.round((doneCount / storyCount) * 100) : null);
    const plannedStart = start || act?.firstActiveSprintStart || '';
    ...
    return {
      ...
      elapsedPct,
      deliveryPct,
      confidenceLabel: !end && storyCount ? 'Children only' : confidenceLabel,
      missingDates: !end,
      childHint: storyCount ? `${doneCount}/${storyCount} children` : 'No child stories',
      childTotal: storyCount,
      childDone: doneCount,
    };
  });
