...
import { writeSquadSprintTruth } from './Delivera-Governance-Squad-Sprint-Truth-02Store-IO.js';
 ...
 */
export async function resolveSnapshotProjects() {
...
async function refreshCurrentSprintSnapshots() {
    ...
    try {
        ...
        const { boards: discoveredBoards } = await discoverBoardsWithCache(snapshotProjects, agileClient);
        const boards = filterSprintCapableBoards(discoveredBoards);
        logger.debug('Current-sprint snapshot projects resolved', {
          projects: snapshotProjects,
          boardCount: boards.length,
          skippedNonScrum: discoveredBoards.length - boards.length,
        });
