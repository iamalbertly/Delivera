...
import { searchIssuesJql } from './Delivera-Jira-Search-01SSOT.js';
...
/**
 * Broad Jira epic fetch for slide matching — no quarter filter (trust: don't hide existing epics).
 */
export async function proposeFromJiraEpicsBroad({ projects, version3Client, quarter = '' }) {
  const pks = normalizeProjects(projects);
  if (!version3Client && !pks.length) return [];

  const agileEpics = await fetchBoardEpicsForProjects(pks, {
    quarter: '',
    financialYearOnly: false,
    maxPerBoard: 120,
  });
  if (agileEpics.length) return agileEpics;

  const candidates = [];
  for (const pk of pks.slice(0, 5)) {
    const { issues } = await searchIssuesJql(version3Client, {
      jql: `project = ${pk} ORDER BY updated DESC`,
      maxResults: 80,
    ...
    for (const issue of issues) {
      ...
      if (quarter && !epicMatchesQuarter(summary, quarter)) continue;
      candidates.push({ issueKey: key, title: summary, summary });
    }
  }
  return candidates;
}
...
/**
 * Slide matcher pool: unfiltered board + quarter-filtered + broad Jira (speed + trust).
 */
export async function buildSlideMatcherEpicPool({ projects, cache, version3Client, quarter = '' }) {
  const pks = normalizeProjects(projects);
  const byKey = new Map();
  const add = (e) => {
    const k = String(e.issueKey || '').toUpperCase();
    if (!k) return;
    byKey.set(k, {
      issueKey: k,
      title: e.title || e.summary || k,
      summary: e.summary || e.title || k,
    });
  };

  // P0: Agile board epics (SD-5314…5317 live here; JQL issuetype=Epic is 410/empty on tenant).
  const agileAll = await fetchBoardEpicsForProjects(pks, { quarter: '', financialYearOnly: false });
  for (const e of agileAll) add(e);
  if (quarter) {
    const agileQ = await fetchBoardEpicsForProjects(pks, { quarter, financialYearOnly: true });
    for (const e of agileQ) add(e);
  }

  if (cache) {
    const unfiltered = await proposeFromBoardCache({ projects: pks, cache, quarter: '' });
    for (const c of unfiltered.candidates || []) add(c);
    if (quarter) {
      const filtered = await proposeFromBoardCache({ projects: pks, cache, quarter });
      for (const c of filtered.candidates || []) add(c);
    }
  }

  if (version3Client && byKey.size < 8) {
    for (const e of await proposeFromJiraEpicsBroad({ projects: pks, version3Client, quarter: '' })) add(e);
    if (byKey.size < 12) {
      const fb = await proposeFromJiraFallback({ projects: pks, version3Client, quarter });
      for (const c of fb.candidates || []) add(c);
    }
  }
...
export async function proposeFromJiraFallback({ projects, version3Client, quarter = '' }) {
  ...
  const agileQ = await fetchBoardEpicsForProjects(pks, { quarter, financialYearOnly: Boolean(quarter) });
  for (const e of agileQ) {
    candidates.push(toCandidate(
      { issueKey: e.issueKey, title: e.title, squad: String(e.issueKey).split('-')[0] },
      'agile-board-epics',
      { confidence: epicMatchesQuarter(e.title, quarter) ? 0.85 : 0.7 },
    ));
  }
  if (candidates.length) method = 'agile-board-epics';
  ...
  for (const pk of pks.slice(0, 5)) {
    ...
    for (const issue of issues) {
      const key = issue?.key || '';
      const summary = issue?.fields?.summary || '';
      if (!summary || !PARTIAL_EPIC_RE.test(summary)) continue;
      ...
      if (matchFv || matchLabel || titleMatch || !quarter) {
