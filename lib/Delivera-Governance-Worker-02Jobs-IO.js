...
export { groupInboxByFingerprint } from '../public/Delivera-App-Governance-Inbox-Group-01SSOT.js';
...
const INBOX_TYPES = new Set(['brief', 'nudge', 'pi-drift', 'confirm', 'impact', 'po-readiness', 'intervention', 'escalation']);
...
/**
 * Mark long-running jobs as interrupted after a crash/restart.
 * @param {{ maxAgeMinutes?: number }} [options]
 * @returns {Promise<number>} count of jobs updated
 */
export async function sweepStaleRunningJobs({ maxAgeMinutes = 15 } = {}) {
  const all = await readJsonl(JOBS_FILE);
...
export async function readLastCompletedBriefJob({ project = null } = {}) {
  const jobs = await readRecentJobs({ project, limit: 20, type: 'prepare-weekly-brief' });
  return jobs.find((j) => j.status === 'completed') || jobs.find((j) => j.status === 'skipped') || null;
}

export async function resolveInboxItem(id, { resolution = 'dismissed', editedContent = '', userId = 'unknown', dismissReason = '' } = {}) {
  const all = await readJsonl(INBOX_FILE);
  ...
  const updated = {
    ...
    staleDismissCount,
    dismissReason: resolution === 'dismissed' ? String(dismissReason || '').slice(0, 64) : item.dismissReason || null,
  };
  all[idx] = updated;
  await mkdir(DATA_DIR, { recursive: true });
...
export function groupInboxByType(items = []) {
  return {
    ...
    intervention: items.filter((i) => i.type === 'intervention'),
    escalation: items.filter((i) => i.type === 'escalation'),
  };
}
