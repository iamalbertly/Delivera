 ...
 */
import { ATTENTION_STATES, formatMetricsStrip, formatPromiseCount } from './Delivera-Governance-GovernanceState-01SSOT.js';
...
function buildBaselineProvenance({ brief = {}, baselineMissing = false, summary = {}, periodKey = '' } = {}) {
  const bc = brief.baselineComparison || {};
  const piName = bc.piName || brief.meta?.quarter || periodKey || 'PI baseline';
  const uploaded = formatDate(bc.baselineDate || bc.approvedAt);
  if (baselineMissing) {
    const quarter = periodKey || brief.meta?.quarter || 'this quarter';
    return {
      available: false,
      line: `No ${quarter} PI baseline slide on file — upload your PowerPoint plan slide to verify commitments`,
      quarter,
      extracted: 0,
      linked: 0,
      unsupported: 0,
  ...
  const s = bc.summary || {};
  const extracted = Number(summary.total) || Number(s.totalCommitted) || 0;
...
function buildHeadline({ judgment = {}, baselineProvenance = {}, periodKey = '', zeroRisk = false, baselineMissing = false, boardUnresolved = false, anchorKey = '' } = {}) {
  if (zeroRisk) {
  ...
  if (baselineMissing) {
    return (focus?.totalCount || 0) === 0
      ? `Upload ${quarter} PI baseline slide to verify ${name} commitments`
      : `${name}${period} commitments cannot be verified yet — upload PI baseline slide`;
  }
  if (boardUnresolved) {
    return `${name} board is not mapped — align board work in Alignment Studio`;
  }
  if (focus.attentionState === ATTENTION_STATES.CANNOT_VERIFY && focus.boardResolved === false) {
    return `${name} board is not mapped — align board work in Alignment Studio`;
  }
  if (focus.attentionState === ATTENTION_STATES.CANNOT_VERIFY) {
    return `${name}${period} commitments cannot be verified yet — upload PI baseline slide`;
  }
  if (unsupported > 0) {
    return `${name} needs one decision. ${unsupported}${period} commitment${unsupported === 1 ? '' : 's'} need attention.`;
  }
...
export function buildPriorityBrief({
  ...
  const baselineProvenance = buildBaselineProvenance({ brief, baselineMissing, summary, periodKey });
  ...
  const anchorInsight = (decision.insights || brief.squadInsights || []).find(
    (i) => String(i.projectKey).toUpperCase() === String(anchorKey).toUpperCase(),
  );
  const boardUnresolved = !baselineMissing && anchorInsight?.boardResolved === false;
  ...
  return {
    ...
    primaryActionTarget: baselineMissing
