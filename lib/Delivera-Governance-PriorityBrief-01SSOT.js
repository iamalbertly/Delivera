...
function buildHeadline({ judgment = {}, baselineProvenance = {}, periodKey = '', zeroRisk = false, baselineMissing = false, boardUnresolved = false, anchorKey = '', summary = {}, portfolioGeneral = false } = {}) {
  if (zeroRisk) {
  ...
  if (portfolioGeneral || String(anchorKey).toUpperCase() === PORTFOLIO_ALL) {
    const atRisk = judgment.atRisk?.length || judgment.offPlanCount || 0;
    ...
    const squadsWithoutBaseline = totalSquads - squadsWithBaseline;
    ...
    if (atRisk > 0) {
      ...
      const gap = Number(judgment.leadingSquad?.notPlannedCount)
        ...
        || 0;
      if (gap > 0) {
        return `Portfolio: ${atRisk} squad${atRisk === 1 ? '' : 's'} need attention — ${lead} tops the board-gap list (${gap} PI commitment${gap === 1 ? '' : 's'} with zero stories on selected boards).`;
      }
      return `Portfolio: ${atRisk} squad${atRisk === 1 ? '' : 's'} need attention — ${lead} needs review first.`;
    }
  ...
  if (unsupported > 0) {
    ...
    if (notPlanned > 0 && notPlanned >= unsupported) {
      return `${name} has ${notPlanned} PI commitment${notPlanned === 1 ? '' : 's'} in Jira with zero stories on selected boards.`;
    }
...
function portfolioHasNoBaselineLine(brief = {}, periodKey = '') {
  ...
  const withBaseline = allProjects.filter((k) => ready[k]?.hasBaseline);
  const withoutBaseline = allProjects.filter((k) => !ready[k]?.hasBaseline);
