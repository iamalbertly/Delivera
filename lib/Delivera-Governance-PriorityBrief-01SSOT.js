...
function buildHeadline({ judgment = {}, baselineProvenance = {}, periodKey = '', zeroRisk = false, baselineMissing = false, boardUnresolved = false, anchorKey = '' } = {}) {
  ...
  const name = (boardUnresolved ? anchorSquad : leading)?.squadName
    || (boardUnresolved ? anchorSquad : leading)?.projectKey;
  const quarter = periodKey || baselineProvenance.quarter || 'this quarter';
  const period = periodKey ? ` ${periodKey}` : '';
  const unsupported = (boardUnresolved ? anchorSquad : leading)?.unsupportedCount || baselineProvenance.unsupported || 0;
  ...
  if (leading.attentionState === ATTENTION_STATES.CANNOT_VERIFY && leading.boardResolved === false) {
    return `${name} board is not mapped — align board work in Alignment Studio`;
  }
  if (leading.attentionState === ATTENTION_STATES.CANNOT_VERIFY) {
    return `${name}${period} commitments cannot be verified yet`;
  }
  if (unsupported > 0) {
