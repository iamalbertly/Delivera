...
- **Frontend (`public/report.html`, `public/current-sprint.html`, `public/leadership.html`, `public/styles.css`, and modular `Delivera-*` scripts)**
  ...
  - `Delivera-Shared-Global-Nav.js` – injects left sidebar (Brief · Sprint · Proof) and orchestrates `ensureTopChrome()`; settings removed from sidebar primaries (gear in top bar). Single source of truth for nav markup.
  - `Delivera-Shared-Top-Chrome-01Render-UI.js` – SSOT for `#app-top-chrome` (Jira-style top bar): toggle, surface switcher, brand/context, contextual search, Create (`data-top-action="create-work"`), notifications, help, settings, avatar. CSS: `public/css/12-top-chrome.css`. Layout tokens: `--top-chrome-height`, `--sticky-global-nav-top`.
