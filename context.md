...
- **Frontend (`public/report.html`, `public/current-sprint.html`, `public/leadership.html`, `public/styles.css`, and modular `Delivera-*` scripts)**
  ...
  - `Delivera-Shared-Top-Chrome-01Render-UI.js` – SSOT for `#app-top-chrome` (Jira-style top bar): toggle, surface switcher (primary wayfinding), brand/context, contextual search, Create (`data-top-action="create-work"`), in-place notification dock, help, settings, avatar (`GET /api/session-meta.json` initials). Default collapsed sidebar on Brief/Proof first visit. CSS: `public/css/12-top-chrome.css` (included in `scripts/build-css.js` + `scripts/check-css.js`). Layout tokens: `--top-chrome-height`, `--sidebar-width`.
...
- **Governance Layer (Vodacom Delivery Accountability)** — deterministic, evidence-bound delivery governance on top of the current-sprint payloads. Algorithms/templates are the always-working core; an external LLM is an optional, validated advisor that falls back to the template on any error and feeds accepted phrasing back into the templates.
  ...
  - **Frontend (PI intelligence Brief):** `public/governance.html`: `#gov-pi-strip-mount` (`19Render-PIConfidenceStrip-UI.js`), `#gov-sticky-answer-mount`, `#gov-feedback-lab-mount` (`21Render-FeedbackImprovementCenter-UI.js`), `#gov-epic-hygiene-mount` (`20Render-EpicHygienePanel-UI.js`). Scope intelligence drawer `18Render-ScopeIntelligenceDrawer-UI.js`; hover proof `22Render-HoverProofCards-UI.js`; global agent bar `GlobalAgentBar-01UI.js` + bootstrap on settings/report. Command bar: PI forum copy, protect-me wording, narration badge. Portfolio 5+ squads: comparison tray filters + pin tile. `Delivera-App-Governance-Brief-Tooltip-01SSOT.js`. Above-fold flow unchanged from command surface; measurement strip includes ad-hoc epic watcher. `public/css/09-governance.css`, `11-executive-pulse.css` (dot grid, shadows).
