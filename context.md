...
- **Governance Layer (Vodacom Delivery Accountability)** — deterministic, evidence-bound delivery governance on top of the current-sprint payloads. Algorithms/templates are the always-working core; an external LLM is an optional, validated advisor that falls back to the template on any error and feeds accepted phrasing back into the templates.
  ...
  - **Phase 3.9 finish (above-fold order):** `governance.html` DOM: answer → `#gov-action-clusters-mount` → setup debt → verdict (`.gov-verdict-fold` when clusters) → `#gov-pi-strip-mount`. Trusted PI strip hides `.gov-pi-counter-row`; setup debt `compact` when owner clusters; cluster issue keys use `gov-issue-key-link` only (no external Jira href). Tests: viewport nudge &lt;900px, baseline propose 500 drawer copy, E12 no `issueUrl` href.
