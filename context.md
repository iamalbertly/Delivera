...
- **Server (`server.js`)**
  - Depends on `express`, `dotenv`, `jira.js`, and internal libs:
    ...
    - `lib/cache.js` – TTL cache for preview responses (memory default; optional Redis when `CACHE_BACKEND=redis` + `REDIS_URL` — see `README.md`). Namespaces: `governanceBrief`, `portfolioDecision` (age-tier TTL via `lib/Delivera-Cache-AgeTier-01TTL-SSOT.js`; keys `governanceBriefCacheKey` / `portfolioDecisionCacheKey`). Client bridges: `Delivera-Shared-Brief-Client-Cache-01Bridge.js`, `Delivera-Shared-Portfolio-Decision-Client-Cache-01Bridge.js`. Preview reuse is per-process with memory-only backends.
...
- **Portfolio deduplication + decision intelligence (2026-06):** `/governance` uses one page controller and one brief load path (`Page-03Load-Controller.js` → `Page-06Portfolio-Render-Plugin.js` renders signal, affected commitments, prepared actions, carousel, decision rail). **Exposure SSOT:** `Delivera-Governance-PortfolioExposure-01SSOT.js` (gap classification, commitments, prepared actions, peer narrative). **Decision contract:** `Delivera-Governance-PortfolioDecision-01SSOT.js` adds `narrative`, `aboveFold`, `affectedCommitments`, `preparedActions`, `decisionProgression`, `decisionBasis`, impact-preview `decisionOptions`. UI: `Portfolio-Commitments-01Render-UI.js`, `Portfolio-PreparedActions-01Render-UI.js`. Recommendation ladder: insufficient proof → confirm scope → validate delivery → review investment. Scope bar: `ScopeBar-01Render-UI.js` + `03Shared-Kernel-SSOT.js` + `04Portfolio-Mode-Render-UI.js`. Intervention client: `InterventionCase-02Client-SSOT.js`. Nav: `Delivera-Shared-Page-Route-01Resolve-SSOT.js`. Retired modules prefixed `DeleteThisFile_`. **SIZE-EXEMPT backlog:** `routes/api.js`, `lib/currentSprint.js`, `Delivera-Work-Draft-Canvas.js`, Report/CurrentSprint public modules >300 lines.
  - **Brief SSOT (loading, scope, cache):** `public/Delivera-Governance-Brief-Page-02Loading-State.js`; `public/Delivera-Shared-Brief-Client-Cache-01Bridge.js`; `public/Delivera-Shared-Scope-Notify-01Bridge.js`; `GOVERNANCE_QUARTER_KEY` in `Delivera-Shared-Storage-Keys.js`; `partitionBriefSurfaces` in `Delivera-App-Governance-Brief-06Surface-Dedupe-SSOT.js`. Tests: `tests/Delivera-Governance-Brief-SSOT-Loading-Scope-Validation-Tests.spec.js` (`npm run test:journey:brief-ssot`) — portfolio-primary on `/governance`.
  ...
  - **Phase 3.9 (projects SSOT, cache-first, unified actions):** `public/Delivera-Shared-Projects-Catalog-01SSOT.js` (compile-time fallback) + `data/Delivera-Org-Project-Catalog.json` (admin-editable) via `lib/Delivera-Org-Project-Catalog-01IO-SSOT.js`; display resolver `public/Delivera-Shared-Project-Display-01Resolve-SSOT.js` + `lib/Delivera-Shared-Project-Display-01Resolve-SSOT.js`; catalog hydrate SSOT `public/Delivera-Shared-Projects-Catalog-01Hydrate-SSOT.js`; Settings hub `public/settings.html` + `Delivera-Settings-Page-01Controller.js`, `Delivera-Settings-MyWorkspace-01Panel-UI.js`, `Delivera-Settings-Organization-01ReadOnly-Panel-UI.js`, `Delivera-Settings-Integrations-01Health-Panel-UI.js`; APIs `GET /api/settings/org-summary.json`, `GET /api/settings/runtime.json`; `GET /api/projects-catalog.json` adds `displayMode`, `catalogSource`, `shortLabel`; admin runbook `docs/org-settings.md`. Tests: `Delivera-Settings-MasterPlan-Logcat-Realtime-Validation-Tests.spec.js`.
  - **Public API (all `requireAuth`):** `GET /api/governance-brief.json?projects=MPSA,MAS` (age-tier TTL via `Delivera-Cache-AgeTier-01TTL-SSOT.js`; on Jira failure serves stale via `cache.getWithStaleFallback` with `confidenceLimit:'stale'` and clamped confidence, else 502 `GOVERNANCE_BRIEF_FAILED`). `POST`/`GET /api/governance/portfolio-decision.json` (cached `portfolioDecision` namespace; invalidated on confirm + intervention mutations). `GET /api/governance/intervention-shortlist.json` (lean topRisks, reuses the brief). `POST`/`GET /api/governance/pi-baseline`; `GET /api/governance/pi-baseline/propose?quarter=FY27+Q1`; `POST /api/governance/pi-baseline/propose-from-image` (base64 slide, user AI headers). `GET /api/governance/scope-intelligence.json`, `GET /api/governance/pi-confidence.json`, `GET /api/governance/feedback-summary.json`, `POST /api/governance/feedback-triage`. `GET /api/governance/inbox.json`, `POST /api/governance/inbox/:id/resolve` (optional `dismissReason`), `GET /api/governance/jobs.json`, `GET /api/governance/worker-receipt.json` (jobs + inbox + cached brief meta; no extra Jira). `GET`/`POST /api/governance/profile`. `GET /api/governance/impact-pack.json`. `POST /api/governance/narration-feedback` (learning loop). `POST /api/governance/adoption-metric` + `GET /api/governance/adoption-metrics.json`. Brief `meta`: `workerReceipt`, `sinceLastRun`, `setupGaps`, `partialProjects`, `commandAnswerSentence`, `epicHygiene`, `adHocEpics`, `piConfidence`, `scopeIntelligence`, `piForumAnswer`, `protectMeAnswer` from worker + PI SSOT modules. Logging is counts-only (projects, board/risk counts, narratedBy) — never issue bodies.
...
- **Gates (11):** `h`, `v`, `dup`, `mvoid`, `fold`, `hvoid`, `overlap`, `sticky`, `shot`, `nvoid`, `stateMatrix`, plus MCP enrichment gate on Phase 0 log.
- **Evidence:** `runs/{id}/evidence-digest.md` + `evidence-bundle.json` with v5 report types; `Build-EvidenceDigest` before cursor phases.
- **MCP:** Playwright MCP via `.cursor/mcp.json` + `scripts/Delivera-Playwright-MCP-01Launch.cmd`; `Invoke-McpPreflight` before collectors; hard-fail unless `-AllowDegradedEvidence`.
- **Prompts:** `.autohacker/prompts/00-03` — 3-phase Cursor workflow (explore → investigate → master plan → build); MCP mandatory.
- **Journey test SSOT:** `journey.governance-autohacker-v6` → `tests/Delivera-Governance-AutoHacker-v6-LayoutHiddenFold-Realtime-Validation-Tests.spec.js`; `npm run test:journey:autohacker` aliases v6.
- **Thresholds:** `maxHiddenValueCount: 8`, `maxLeftWhitespaceRatio: 0.55`, `minExplorationIdeas: 20` in `.autohacker/config/values.json`.
- **Target journeys:** `/governance`, `/current-sprint?boardId=6` in `targets.json` `journeyUrls`.

### Impact Cockpit / Evidence OS Vertical Slice (2026-06-22)

- **Route and UI:** `/impact` (with `/evidence` still available for compatibility) uses the shared top chrome/sidebar and `Delivera-Core-UI-02Primitives-Modal.js` drawer behavior. No duplicate navigation or modal framework was introduced.
- **Product stance:** Jira, SuccessFactors, Teams/email, and governance documents remain source systems. Delivera presents an action/decision cockpit and keeps evidence, commitments, validation, and report snapshots as background infrastructure.
- **API:** `routes/evidence-os.js` owns `/api/evidence-os/*` and is mounted before the legacy API router. Existing Brief/Sprint/Proof routes remain unchanged.
- **Data boundary:** checked-in Postgres migration at `db/migrations/0001_evidence_os_foundation.sql`, Drizzle schema at `db/schema/evidence-os.js`, and local fallback store at `data/Delivera-EvidenceOS-Store.json` when `DATABASE_URL` is absent.
- **Service modules:** identity/RBAC, audit, evidence, contribution, goals, validation, report snapshots, safe risk language, opportunity scoring, and development support live under `lib/Delivera-*-01*.js` with existing naming style.
- **Cockpit layer:** `Delivera-EvidenceOS-Insight-01Service.js` builds `What needs my attention`, linked source commitments, portfolio intelligence, value generation/protection/capability streams, Nuru brief, structured proof, validation prompt drafts, and agent activity.
- **Governed AI:** new orchestrator task types `contribution-draft`, `report-audience-summary`, and `claim-gap-detection` reuse the existing AI router, budget/redaction, provider settings, and output verifier. Contribution drafts follow the six-part proof model: situation, my action, stakeholders, result, strategic relevance, evidence. Outputs persist only as Tier 4 evidence and cannot verify outcomes.
