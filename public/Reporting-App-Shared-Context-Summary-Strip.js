...
function renderChip(chip) {
  ...
  const action = chip?.action ? ` data-context-action="${escapeHtml(chip.action)}"` : '';
...
export function renderContextSummaryStrip({ title = '', chips = [], secondary = '', actions = [] } = {}) {
  ...
  const actionsHtml = (Array.isArray(actions) ? actions : [])
    ...
    .join('');
  return ''
    + '<section class="context-summary-strip" data-context-summary-strip="true" aria-label="Current context">'
