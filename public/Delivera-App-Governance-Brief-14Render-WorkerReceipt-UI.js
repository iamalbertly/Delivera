...
export function renderWorkerReceiptRail(brief, feedbackSummary = null) {
  ...
  return `
    <div class="gov-worker-receipt-wrap" role="status" aria-live="polite">
      <p class="gov-worker-receipt${auth ? ' gov-worker-receipt--warn' : ''}">
      ...
      ${since ? `<p class="gov-worker-since-line gov-since-delta" data-hover-proof="since-last-run">${escapeHtml(since)}</p>` : ''}
