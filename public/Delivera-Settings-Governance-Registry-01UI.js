...
function render() {
  ...
  const bulkFirst = canManageOrganizationSettings
    ? `<details class="registry-band registry-band--org-policy" data-registry-org-policy="1"><summary class="registry-band-head"><div><h3>Organization participation policy</h3><p>Toggle pending-consent and operational exceptions once for every Delivera surface.</p></div><span data-selected-count>0 squads selected</span></summary><div class="registry-bulk" aria-labelledby="registry-bulk-title"><div class="registry-bulk-body"><p id="registry-bulk-title">Updates are atomic and auditable across Governance, Current Sprint, and Actions.</p><button type="button" class="btn btn-link btn-compact" data-select-pending aria-pressed="false">Select pending consent</button><label>New participation<select data-bulk-participation><option value="">Keep current</option><option value="pi-governed">PI-governed (soft include)</option><option value="pending-consent">Pending consent</option><option value="operational-exception">Operational exception (soft exclude)</option></select></label><label>Reason<input data-bulk-reason autocomplete="off" data-1p-ignore data-lpignore="true" placeholder="Why this organization policy is changing"></label><div data-bulk-diff class="registry-policy-diff" role="status" aria-live="polite">Select squads to preview an organization-wide change.</div><button type="button" class="btn btn-primary" data-bulk-preview disabled>Publish changes</button><p data-bulk-status role="status" aria-live="polite"></p></div></div></details>`
    : '';
