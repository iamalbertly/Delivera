/**
 * ALB-76: /current-sprint scroll height + screenshots after context/evidence strip merge.
 * Usage: node scripts/ux-alb76-current-sprint-once.mjs [before|after]
 */
...
async function capture(viewport, name, bust) {
  const url = `${baseUrl}${bust ? `?uxalb76=${bust}` : ''}`;
  ...
  const png = path.join(outDir, `${label}-ALB-76-current-sprint-${name}.png`);
...
const reportPath = path.join(outDir, `ALB-76-gate-${label}.json`);
