/**
 * Direct-Value Master Plan Round 7 — friction kill & trust seal contracts.
 */
...
test.describe('Direct-Value Master Plan Round 7 realtime validation', () => {
  test.describe.configure({ retries: 0 });

  test('@focused direct-value round7 contracts', async ({ page, request }) => {
    ...
    await test.step('05 alignment drawer panel clears top chrome overlap', async () => {
      ...
      await page.locator('.gov-right-drawer-panel [data-baseline-close], .gov-right-drawer-panel [data-drawer-close]').first().click();
    });

    await test.step('06 header and cockpit share aligned blocker key', async () => {
    ...
    await test.step('07 actions inline blocker queue with boards-backed fetch', async () => {
    ...
    await test.step('08 status honesty bar shows delivery blocked chip', async () => {
      await mockRound6Governance(page);
      await page.goto('/governance');
      if (await skipIfRedirectedToLogin(page, test)) return;
      await waitForPortfolioReady(page);
      const bar = page.locator('[data-testid="portfolio-status-honesty-bar"]');
      await expect(bar).toBeVisible();
      await expect(bar).toContainText(/Delivery blocked|PI not aligned/);
    });

    await test.step('09 sprint header owns nudge CTA without stories duplicate', async () => {
      await mockAlignedBlockedSprint(page);
      await page.setViewportSize({ width: 1400, height: 900 });
      await page.goto('/current-sprint');
      if (await skipIfRedirectedToLogin(page, test)) return;
      await page.waitForSelector('.sprint-intervention-item-primary', { timeout: 20000 });
      await expect(page.locator('.sprint-intervention-item-primary')).toContainText(/Nudge|Ping/);
      await expect(page.locator('[data-send-top-nudge]')).toHaveCount(0);
    });

    await test.step('10 commitment fuzzy merge single Next move line', async () => {
    ...
    await test.step('11 ultrawide sprint proof rail visible', async () => {
      await mockAlignedBlockedSprint(page);
    ...
    await test.step('12 blocked sprint alignment strip above fold', async () => {
      await mockAlignedBlockedSprint(page);
      await page.setViewportSize({ width: 1400, height: 900 });
      await page.goto('/current-sprint');
      if (await skipIfRedirectedToLogin(page, test)) return;
      await page.waitForSelector('[data-alignment-above-fold="1"]', { timeout: 20000 });
    });

    await test.step('13 blocked sprint full-sprint fold closed by default', async () => {
      await expect(page.locator('details.sprint-full-sprint-fold').first()).not.toHaveAttribute('open', '');
    });

    await test.step('14 PI wizard DMS Q2 slide shows aligned reconcile state', async () => {
      ...
      await uploadDone;
      await expect(page.locator('[data-testid="gov-baseline-context"]')).toContainText('DMS', { timeout: 15000 });
      await expect(page.locator('[data-testid="gov-baseline-aligned"]')).toBeVisible({ timeout: 15000 });
    });

    await test.step('15 portfolio primary CTA reads Upload PI slide when synergy low', async () => {
      await mockRound6Governance(page);
      await page.goto('/governance');
      if (await skipIfRedirectedToLogin(page, test)) return;
      await waitForPortfolioReady(page);
    ...
    await test.step('16 evidence rail docked not calibration overlay', async () => {
      await mockRound6Governance(page);
      await page.goto('/governance');
      if (await skipIfRedirectedToLogin(page, test)) return;
      await waitForPortfolioReady(page);
    ...
    await test.step('17 grid Next move hidden when portfolio rail visible', async () => {
    ...
    await test.step('18 mobile governance drawer and strip usable at 390px', async () => {
    ...
    await test.step('19 mobile sprint blocker visible above fold at 390px', async () => {
      await page.setViewportSize({ width: 390, height: 844 });
      await mockAlignedBlockedSprint(page);
      await page.goto('/current-sprint');
      if (await skipIfRedirectedToLogin(page, test)) return;
      await page.waitForSelector('.sprint-intervention-item-primary', { timeout: 20000 });
      const box = await page.locator('.sprint-intervention-item-primary').boundingBox();
      expect(box).toBeTruthy();
      expect(box.y).toBeLessThan(700);
    });

    await test.step('20 cross-surface telemetry clean', async () => {
      await mockEmptyActions(page);
      await mockAlignedBlockedSprint(page);
      await page.goto('/actions');
      if (await skipIfRedirectedToLogin(page, test)) return;
    ...
    await test.step('21 stories card visible above fold on blocked load', async () => {
    ...
    await test.step('22 final telemetry guard', async () => {
      assertTelemetryClean(telemetry);
    });
  });
});
