...
test.describe('Governance FirstViewport Value Dedupe MasterPlan @focused', () => {
  test('delivery-first viewport, CTA dedupe, epic rail, continuity @focused', async ({ page }) => {
    ...
    await test.step('04 epic commitment rail visible without drawer', async () => {
      const rail = page.locator('[data-epic-commitment-rail="1"]');
      await expect(rail).toBeVisible();
      await expect(page.locator('[data-epic-rail-chip="1"]').first()).toBeVisible({ timeout: 10000 });
    });

    await test.step('05 top-risk preferred highlight; one today continuity link', async () => {
    ...
    await test.step('06 matrix shows evidenced / diverted / slip columns', async () => {
    ...
    await test.step('07 continuity uses squad param without console conflict', async () => {
      await page.locator('[data-story-squad="SD"]').click();
      await expect(page).toHaveURL(/[?&]squad=SD/);
      await expect(page).not.toHaveURL(/spotlight=SD/);
